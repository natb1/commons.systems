...
assert_approves \
  ...
  "Bash" \
  ".claude/skills/dispatch/scripts/run-lint.sh foo"

assert_approves \
  "absolute path script invocation" \
  "Bash" \
  "/Users/n8/natb1/commons.systems/worktrees/322-test/.claude/skills/dispatch/scripts/sync-issue-context 322"

assert_approves \
  "quoted CLAUDE_PROJECT_DIR prefix" \
  "Bash" \
  '"/Users/n8/project"/.claude/skills/dispatch/scripts/post-pr-comment.sh 323 file.txt'

assert_approves \
  "script with no arguments" \
  "Bash" \
  ".claude/skills/dispatch/scripts/issue-primary"

assert_approves \
  "script in subshell with absolute path" \
  "Bash" \
  '(/Users/n8/project/.claude/skills/dispatch/scripts/run-lint.sh)'
...
assert_approves \
  ...
  "Bash" \
  ".claude/skills/dispatch/scripts/run-qa-cleanup.sh 2>&1"

assert_approves \
  "user-reported: qa-cleanup 2>&1 piped to tail" \
  "Bash" \
  ".claude/skills/dispatch/scripts/run-qa-cleanup.sh 2>&1 | tail -5"

assert_approves \
  "2>/dev/null stderr silence piped to tail" \
  "Bash" \
  ".claude/skills/dispatch/scripts/run-qa-cleanup.sh 2>/dev/null | tail -5"
...
assert_approves \
  ...
  "Bash" \
  ".claude/skills/dispatch/scripts/run-lint.sh && .claude/skills/dispatch/scripts/run-unit-tests.sh"
...
assert_approves \
  ...
  "Bash" \
  ".claude/skills/dispatch/scripts/run-lint.sh 2>&1 | grep -E \"(foo|bar)\" | head -40"

assert_approves \
  "pipe inside single-quoted regex alternation" \
  "Bash" \
  ".claude/skills/dispatch/scripts/run-lint.sh | grep -E '(foo|bar)' | tail -5"
...
assert_approves \
  ...
  "Bash" \
  "echo .claude/skills/dispatch/scripts/run-lint.sh"

assert_approves \
  "cat (in allowedTools) reading a script path" \
  "Bash" \
  "cat .claude/skills/dispatch/scripts/run-lint.sh"

assert_passthrough \
  "absolute script path as argument to rm" \
  "Bash" \
  "rm -rf / /Users/n8/.claude/skills/dispatch/scripts/issue-primary"

assert_passthrough \
  "absolute script path as argument to chmod" \
  "Bash" \
  "chmod 777 /Users/n8/.claude/skills/dispatch/scripts/issue-primary"

assert_passthrough \
  "absolute script path as argument to cp" \
  "Bash" \
  "cp /tmp/evil /Users/n8/.claude/skills/dispatch/scripts/issue-primary"

assert_passthrough \
  "non-Bash tool" \
  "Read" \
  ".claude/skills/dispatch/scripts/run-lint.sh"
...
assert_passthrough \
  "path traversal via .." \
  "Bash" \
  ".claude/skills/dispatch/scripts/../../../evil.sh"

assert_passthrough \
  "path traversal after valid script name prefix" \
  "Bash" \
  ".claude/skills/dispatch/scripts/legit/../../../evil.sh"

assert_passthrough \
  "path continuation through script name as directory" \
  "Bash" \
  ".claude/skills/dispatch/scripts/run-lint.sh/../../evil.sh"

assert_passthrough \
  "command chaining with &&" \
  "Bash" \
  ".claude/skills/dispatch/scripts/run-lint.sh && rm -rf /"

assert_passthrough \
  "pipe to unapproved command" \
  "Bash" \
  ".claude/skills/dispatch/scripts/run-lint.sh | malicious-command"
...
assert_passthrough \
  "semicolon chaining" \
  "Bash" \
  ".claude/skills/dispatch/scripts/run-lint.sh; evil-command"
...
assert_passthrough \
  ...
  "Bash" \
  "$(printf '.claude/skills/dispatch/scripts/run-lint.sh\nrm -rf /')"
...
assert_passthrough \
  "output redirection" \
  "Bash" \
  ".claude/skills/dispatch/scripts/run-lint.sh > /tmp/exfil"

assert_passthrough \
  "input redirection" \
  "Bash" \
  ".claude/skills/dispatch/scripts/run-lint.sh < /etc/shadow"
...
TOTAL=$((TOTAL + 1))
input=$(jq -n '{tool_name: "Bash", tool_input: {command: ".claude/skills/dispatch/scripts/run-lint.sh"}}')
output=$(printf '%s\n' "$input" | "$HOOK" 2>/dev/null) || true
if printf '%s\n' "$output" | jq -e '
