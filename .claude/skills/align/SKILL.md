...
- A node id names a ratification or a review of one; a disposition names
  ...
  4. Review (`recording`, `clean-context-review`). Before the author sees
     the recommendation, `/align-review` reads each recommended node in a
     fresh context of its own (§5) and either forwards it, with its
     strongest counter-argument when it found one and the session's
     reply, or kicks it back to stage 2 or 3 with findings.
...
- **Review in clean context**, before the author sees a recommendation.
  ...
  invoke `/align-review` with their ids (`clean-context-review`): one
  fresh subagent per node (opus, high effort), never a fork, whose brief
  carries nothing but: the node as it would be committed, its ancestry to
  the ceiling, the global-tier nodes, the nodes it cites, the author's
  words, the sitting's other drafts, and what the review judges: whether the answer says what the
