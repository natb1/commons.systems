...
If the named target issue `<N>` is **closed**, or has any **open** blocker (run
`issue-blocking <N>` and check for any entry with `state` `OPEN`): release the
lock (see *Releasing the lock*), report the blocker, apply `dispatch:office-hours`
to the target's PR if one exists (see *Applying `dispatch:office-hours`* below),
then proceed to Step 7 with `notify target-blocked`. The open-blocker guard
applies even when a PR exists; closed blockers do not gate.
...
Pass no `--color` — `dispatch-complete-phase` / #757 own the label's canonical
definition; this call site only needs the label to exist.
...
- **`enter <path>`** → re-use an existing `<issue>-*` worktree (recycle after
  completion, or recycle of an orphan worktree for a queue selection, #905). Set
...
- **`conflict <path>`** → the target already has a worktree owned by another
  session (for a queue selection this arises only from a race). Release the lock
  (see *Releasing the lock*), then proceed to Step 7 with `notify
  worktree-conflict` — the user-visible report is mandatory there ("worktree at
...
- **`notify <reason>`** — `notify spawn-failed` (Step 6's spawn exited
  ...
  self-close — the session stays in `claude agents` until the user closes it.
