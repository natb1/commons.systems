...
3. `dispatch-materialize-spawn` — runs the explicit-path guards, resolves the
   ...
   Emits one **terminal token** (Table 2). A merge conflict spawns a
   `/dispatch-resolve-conflict` bg job (named for the worktree) that auto-resolves
   or escalates; the router does not resolve in-session.
...
| `notify spawn-failed` | `dispatch-spawn-worker` exited non-zero — a worker was spawned but did not register. (Also emitted when a merge-conflict's `/dispatch-resolve-conflict` bg job fails to spawn; the script parked the issue first.) | `notify` |
...
- **`--gap N>1`** (queue mode) — the script fans out internally, spawning up to
  ...
  Section 3 dispositions prescribe. A `target-blocked` parked target was already
  labeled `dispatch:office-hours` by the script; a `merge-conflict` target was
  handed to a `/dispatch-resolve-conflict` bg job (and counts as a filled slot).
