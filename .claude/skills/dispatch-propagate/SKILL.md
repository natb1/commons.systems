...
| `drain ci-reseeded` | **Explicit-target path only.** The single explicit `/dispatch <N>` target's PR CI is still in progress (the script's `dispatch-ci-ready` gate is not-ready; it printed the `#<N>:` line — echo it). A target-keyed reseed timer was scheduled, so the chain returns to `<N>` when its CI concludes: "waiting on #<N>'s CI; reseed scheduled". In multi-target fan-out runs, not-ready targets are skipped by selection's `dispatch-ci-ready` gate before `dispatch-materialize-spawn` is called — this token is never emitted for queue-mode targets. | `drain` |
| `notify ci-wait-exhausted` | **Explicit-target path only.** The CI-wait attempt cap was hit; the issue was parked on `dispatch:office-hours` and no further reseed will be scheduled: "#<N>'s CI did not conclude within the attempt cap; parked for review". | `notify` |

> Fallback: if `dispatch-schedule-target-reseed` hard-fails, the explicit
> single-target path emits `drain ci-waiting` instead (route as `drain`); the
> reason is on stderr.
