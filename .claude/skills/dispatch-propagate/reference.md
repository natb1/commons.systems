...
transient `systemd.user` timer (`dispatch-reseed-<resets_at>`) whose ExecStart
runs `dispatch-tick` from the main worktree. The unit name embeds the epoch, so
a repeated call observing the same blocking cap collides on the unit name and is
a no-op — idempotent across the many ticks that may observe the same stall.
...
is the continuation invariant: a tick that leaves no continuation (no worker
spawned, no pending `dispatch-reseed*` timer, no live busy worker) emits a
one-line stall reason to the journal. The `office-hours-select-target`
script surfaces this stall state so a human can resume the chain. This makes
a stalled chain visible rather than silently losing it to a missed heartbeat
window.
