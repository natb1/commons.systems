...
assert_eq "synth: the synthesized recommendation states clear-park alone is correct once the session is already gone" "yes" \
  "$(case "$(td_park_arg 5)" in *"shows no session for this node"*"clear-park -C <repo-root> <node-id>\` alone is the correct and sufficient action"*) printf 'yes' ;; *) printf 'no' ;; esac)"
td_teardown
