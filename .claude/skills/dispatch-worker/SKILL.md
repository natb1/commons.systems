...
exit fires, proceed to Step 4 with `notify worker-wrong-cwd` — do not derive
a phase or run a phase skill.
...
| `notify worker-wrong-cwd` | Step 0 — branch-name / `<worktree-path>` cross-check failed (spawn cwd mismatched the issue or the positional argument) |
...
The worker was born in its target worktree (spawn cwd) and never entered
