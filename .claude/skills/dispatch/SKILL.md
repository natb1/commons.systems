...
- **`busy`** → the wait timeout elapsed without acquiring — a wedged selection
  ...
  to Step 10** (early-stop) — run no sync, no health gate, no sweep, no selection,
  and no phase skill.

### Releasing the lock

The lock covers **Steps 0-6 only** — target selection and worktree resolution.
Steps 7-10 (the phase skill, the pre-implementation relevance review, and the
...
- **Every Steps 1-6 stop path** — immediately before reporting the stop reason
  and proceeding to Step 10 (early-stop).
...
- If `git fetch` fails, or `git merge --ff-only` rejects a non-fast-forward (local
  ...
  the lock*), surface the error, then **proceed to Step 10** (early-stop) — do
...
- **Issue or PR argument given** → strip any leading `#`, then normalize the
  ...
  - **Non-zero exit** → release the lock (see *Releasing the lock*), report the
    script's stderr message, then **proceed to Step 10** (early-stop); create no
...
- **No argument** → run the `origin/main` CI health gate first, then the
  ...
  - `worktree-closed <N> <branch>` — run from inside a worktree whose issue is
    ...
    and **proceed to Step 10** (early-stop) (consistent with the named-target
    "closed → report and stop" rule in Step 5)
  ...
  the queue is empty, then **proceed to Step 10** (early-stop).
  ...
  **main-broken handler.** `origin/main` itself is red, so no new work is safe
  ...
  then **proceed to Step 10** (early-stop). Once a PR that fixes main exists the
  ...
  **jit-reminder handler.** The JIT scan selected `<num>` in `<repo>` as the
  ...
  4. **Stop the tick — bypass Step 10.** The handler stops directly without
     routing through Step 10: the summary just printed must stay open in the
     transcript for a human to read, and Step 10's terminal disposition would
...
lock*), report it, then **proceed to Step 10** (early-stop).
...
- **`conflict <path>`** → a queue-selected target already has a worktree, so
  ...
  conflict (name `<path>` and issue `<N>`), then **proceed to Step 10**
...
| `done` | non-draft (ready) PR | already complete — report, then Step 10 (early-stop) |
...
- **`waiting`** — CI checks are still running or queued. Monitor them to
  ...
  4. If the re-derived phase is still `waiting` (CI never registered any check),
     report it, then **proceed to Step 10** (early-stop) — do not loop.
...
- **`done`** — report that the PR is already ready, then **proceed to Step 10**
  (early-stop). No phase skill ran, so Step 10 spawns no successor and
  self-closes.
...
After the one phase skill has run to completion, **proceed to Step 10**
(phase-completed) — do not advance to the next phase here; Step 10 hands off and
ends the job.
...
- **`stop`** — codebase has moved past the need; report what changed and recommend
  ...
  **proceed to Step 10** (early-stop).

## 10. Hand off and self-close

Every Step 0–9 termination routes here — Step 10 is the single way a `/dispatch`
...
The one documented exception is the **jit-reminder handler** in Step 4: by
design it bypasses Step 10 and stops directly, because its user-visible summary
must stay open in the transcript for a human to read — self-closing the job
would hide it. See the jit summary session note at the end of Step 4.

Each termination reaches Step 10 with one of two dispositions, named by the step
that routed here:

- **phase-completed** — a phase skill (`/plan-implement`, `/verify-pr`,
  `/dispatch-qa`, `/code-review-fix`, `/review-fix`, or `/security-review-fix`)
  ran to completion. Only the Step 8 post-dispatch hand-off arrives this way.
...
`EnterWorktree` in Step 6 (the `enter` and `create` outcomes), call
...
- **The report surfaces a deviation** — a phase-completed run whose step-2
  ...
  canonical definition — Step 10 only needs the label to exist.
...
- **Clean completion or early-stop** — an early-stop, or a phase-completed run
  ...
  background job) — so an interactive `/dispatch` reaching Step 10 does not stop
