...
- **No argument** → run the `origin/main` CI health gate first, then the
  ...
  **jit-reminder handler.** The JIT scan selected `<num>` in `<repo>` as the
  ...
  no worktree, no PR, no phase skill, no leaf trace. Steps 3, 4, and 5-6 are all
  skipped.

  1. **Claim the issue inside the scoped lock window.** The lock is still held
     from Step 0 — the reminder path is a Step 2 stop path and never reaches
     Step 4's proceed-path release, so the claim runs under the lock, exactly as
  ...
  2. **Release the lock.** This is a Step 1-4 stop path — release the lock (see
