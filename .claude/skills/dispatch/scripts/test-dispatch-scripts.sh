...
rm_log=$(cat "$SPAWN_RM_LOG" 2>/dev/null || true)
assert_eq "happy: 'claude rm handoff01' was invoked (self-close fired)" \
  "handoff01" "$rm_log"
handoff_teardown
...
TOTAL=$((TOTAL + 1))
if [[ ! -e "$SPAWN_RM_LOG" ]]; then
  PASS=$((PASS + 1)); echo "  PASS: spawn-fail: no 'claude rm' invocation recorded"
else
  FAIL=$((FAIL + 1)); echo "  FAIL: spawn-fail: no 'claude rm' invocation recorded"
  echo "    rm-log: $(cat "$SPAWN_RM_LOG")"
fi
handoff_teardown
...
TOTAL=$((TOTAL + 1))
if [[ ! -e "$SPAWN_RM_LOG" ]]; then
  PASS=$((PASS + 1)); echo "  PASS: deviation: no 'claude rm' invocation recorded"
else
  FAIL=$((FAIL + 1)); echo "  FAIL: deviation: no 'claude rm' invocation recorded"
  echo "    rm-log: $(cat "$SPAWN_RM_LOG")"
fi
handoff_teardown
...
rm_log=$(cat "$SPAWN_RM_LOG" 2>/dev/null || true)
assert_eq "early-stop: 'claude rm handoff01' was invoked" \
  "handoff01" "$rm_log"
handoff_teardown
...
TOTAL=$((TOTAL + 1))
if [[ ! -e "$SPAWN_RM_LOG" ]]; then
  PASS=$((PASS + 1)); echo "  PASS: missing-pr: no 'claude rm' invocation recorded"
else
  FAIL=$((FAIL + 1)); echo "  FAIL: missing-pr: no 'claude rm' invocation recorded"
  echo "    rm-log: $(cat "$SPAWN_RM_LOG")"
fi
handoff_teardown
