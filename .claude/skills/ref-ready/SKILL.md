...
Rationale: `/implement` opens exactly one PR per issue (`Closes #N` is the implement→fix-checks transition marker), so a multi-PR leaf issue breaks the 1:1 issue→PR mapping. The gate is structural, not stylistic.
