...
parallel node-lane check keys on that marker: if `reviewed` is present in
`execution.markers` of the already-fetched `NODE_MD` (parsed from `origin/main`
in the preamble), this is an interrupted prior run — **skip Steps 1–6** and go
straight to Step 7's terminal flush, exactly as the label check routes the issue
lane. The `reviewed` marker is the node lane's terminal action and is already
