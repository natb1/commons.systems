...
1. **Read the population**, capturing stdout and stderr COMBINED:
   ...
   ```bash
   npx tsx packages/intentionsutil/scripts/office-hours-select.ts --list 2>&1
   ```
...
5. **Report the two totals**, from two different sources — do not conflate them:
   - **Parked total** is simply the row count from step 2. Verify it against `npx tsx packages/intentionsutil/scripts/office-hours-select.ts --list | wc -l` — the `--list` output has no header and no blank lines, so the two must match exactly.
