...
3. **Read cheaply via targeted jq slices** of `tmp/usage-audit.json`. Do NOT `cat` the whole file into context — read only the slices needed for each ranking step:
   ...
   `window.sidecar_eligible`/`sidecar_present`/`sidecar_present_rate` measure how much of this sidecar-driven join actually landed. **`sidecar_eligible` counts the worker sessions the stamping gate COULD have stamped — not all worker sessions.** A session is eligible when it was observed on at least one branch `dispatch-stamp-session`'s worker-branch gate accepts (`^[0-9]+-` for issue branches, `^(graph|tactic|strategy)-.` for graph-native ones), or when it already carries a sidecar — an existing sidecar is direct proof of stamping, supersedes the branch inference, and is what makes `sidecar_present_rate <= 1` true by construction. The worker sessions the gate would refuse are carried by two companion counters rather than dropped: `window.sidecar_ineligible_unstampable_branch` (at least one observed branch, none passing the gate — chiefly the sessions `dispatch-graph-execute` spawns with `--cwd $PROJECT_ROOT`, whose tree sits on `main`) and `window.sidecar_ineligible_branch_unknown` (no observed branch at all, so the gate cannot be evaluated either way). The three PARTITION the worker population: `sidecar_eligible + sidecar_ineligible_unstampable_branch + sidecar_ineligible_branch_unknown` equals the worker-session count. Read a nonzero `sidecar_ineligible_unstampable_branch` as a scope statement, not a stamping regression. Three further companion fields govern how to read the rate: `window.sidecar_coverage_measurable` (true at fleet/session scope, false at `--node` scope), and `window.scope_filter_dropped_unstamped`/`scope_filter_dropped_other_node` (what the `--node` scope filter dropped before those sessions ever reached `.sessions[]`). Fleet-scope reads the rate as a genuine coverage measurement; a `--node` run reads it as a structural artefact instead and consults the drop counters — see `aggregate-usage.sh`'s BEHAVIOR CONTRACT header for the full field contract (why `--node` coverage collapses to present==eligible by construction, and why the drop counters count candidate transcripts rather than worker sessions).
...
6. **Land the top-N ranked opportunities in the graph ledger.** A ranked opportunity that exists only in the markdown report the next step emits is exactly the findings-in-prose-only defect `strategy-recursive-self-improvement` names: the report alone gives the ordinary dispatch queue nothing to execute. Before the report is emitted, land the top **N = 5** ranked opportunities from step 5's ranking (the highest `price_proxy_usd` magnitude entries, same order the report itself will use) as graph ledger tactics through `.claude/skills/dispatch-propagate/scripts/dispatch-eval-finding`, passing `--sensor rsi-audit`.
   ...
   **Find-or-create, not append-only — this is the entire point of the writer.** `dispatch-eval-finding` creates the graph node `tactic-eval-finding-<slug>` on first sight of a finding and, on every later run where the SAME finding recurs, updates that SAME node instead of minting a second one: it refreshes the body and increments `attributes.measured_impact`'s `recurrence_count` record. A finding that lands in this week's top 5 and again in next week's top 5 is ONE ledger tactic with a rising recurrence count, not two. Reading the ledger first (below) before choosing a slug is mandatory: minting a near-duplicate slug for a finding that already has an entry — open OR retired, since a retired entry RESUMES its count on recurrence rather than restarting at 1 — defeats the whole mechanism.
   ...
   1. **Read the existing ledger**: `.claude/skills/dispatch-propagate/scripts/dispatch-eval-finding --list --like '<this opportunity's one-line statement>'` — prints the mint-or-reuse candidate population (open and retired entries) as JSON. **Always pass `--like`**: membership is the whole open tactic population in no namespace, so bare `--list` prints ~490 rows / ~250 KB, and this step runs once per opportunity. `--like` ranks by lexical overlap with the statement in hand and emits the top `--limit` rows (default 40) plus every row carrying measurements whatever its score, disclosing what it elided on stderr as `population=N emitted=N elided=N score_cut=F cut_ties_elided=N`; widen it with `--limit <n>` if the shortlist is too narrow to judge against, rather than dropping the bound. `score_cut` is NOT a threshold — rows tie at it in bulk and the survivors are picked by id order rather than relevance, so a non-zero `cut_ties_elided` counts equal-ranked rows dropped arbitrarily and is itself a reason to widen. An empty `--like ""` is refused, not treated as absent. Judge whether this occurrence IS one of the listed findings (same lens, same root cause) before choosing a slug. There are exactly three outcomes and each has exactly one action — a match you cannot address is NOT "nothing listed matches", so never reach the third by elimination:

      - **It matches a row whose `addressable_by` is `slug`** (open or retired) → reuse that existing slug in step 6.5 and continue.
      - **It matches a row whose `addressable_by` is `id`** → **skip this opportunity's write entirely: mint nothing, and do not run step 6.5 for it.** This is the common case, not a corner one — membership is the whole open tactic population and only the `tactic-eval-finding-` prefix carries slugs (39 of 493 rows, measured 2026-08-31). No flag reaches an id-addressed row yet: `--id` arrives with Unit 4 of `intentions/tactic-eval-finding-ledger.md`, which has not landed. Skip to step 6.6 and record the outcome as `unaddressable-id-match (<the matched row's id>)` in the report's "Ledger-landing outcomes" subsection, then move to the next ranked opportunity. Minting a `tactic-eval-finding-<slug>` for a finding you have already matched lands a second graph node for one finding and freezes the recurrence figure on the real record — the exact find-or-create defeat `:164` warns about.
      - **Nothing listed is this finding**, whatever its `addressable_by` → choose a new slug in step 6.2.
   ...
   6. **Record the outcome.** The script prints exactly one word on a successful exit: `landed` (a new occurrence committed and verified on `origin/main`), `noop` (nothing needed writing — most often the in-flight guard, because the entry's `execution` is non-null and a PR already owns its lifecycle), or `skipped-locked` (another writer held the graph-write mutex after the bounded wait; this occurrence is LOST, not deferred, and the next run's `--list` read is where it would be caught again). A non-zero exit (`1` write failed and rolled back, `64` usage, `69` environment, `70` write failed with a dirty node file requiring the named `git checkout --`) is a landing failure, not a skip — surface the exit code and stderr in the report's ledger-landing subsection rather than dropping the finding silently. An opportunity that step 6.1 resolved to an id-only match never reaches the script at all: record it in that same subsection as `unaddressable-id-match (<the matched row's id>)`, which is the only record that the recurrence was seen.
...
7. **Emit the ranked markdown report.** Structure it as follows:
   ...
   - **Ledger-landing outcomes**: a short subsection listing each of the N opportunities landed in step 6 — its slug, the `tactic-eval-finding-<slug>` id, and its landing outcome (`landed` / `noop` / `skipped-locked` / `unaddressable-id-match (<id>)` / a failure exit code). This is the audit trail that a ranked opportunity actually reached the graph, not only this report.
...
The same sidecar backs `window.sidecar_eligible`/`sidecar_present`/`sidecar_present_rate`. Step 3 above describes the eligibility contract — that `sidecar_eligible` is the STAMPABLE worker population rather than all workers, that `window.sidecar_ineligible_unstampable_branch` and `window.sidecar_ineligible_branch_unknown` carry the rest, and that the three partition the worker population — and how `window.sidecar_coverage_measurable` and the `window.scope_filter_dropped_*` counters change what the rate means at `--node` scope versus fleet/session scope. See `aggregate-usage.sh`'s BEHAVIOR CONTRACT header for the full field contract.
