...
function buildPlanPrompt(strategy, tactic, gather, mode) {
  return [
    ...
    '  dir. For a NESTED workspace the dir is the only valid value and the bare',
    '  basename matches nothing: `packages/intentionsutil`, `packages/ds`,',
    '  `packages/blog` and `artifacts/plan-view` are valid, while',
    '  `intentionsutil`, `ds`, `blog` and `plan-view` are not. For a TOP-LEVEL',
    '  workspace the dir IS its own basename and is valid unprefixed —',
    '  `landing`, `print`, `office-hours`, `office-hours-snapshot` — so never',
    '  invent a `packages/` prefix for one. Validate the name against the FILTERED',
    '  project',
