...
function buildDecomposePrompt(strategy, drafts, gather, drift) {
  return [
    ...
    '  (SCHEMA.md, package READMEs), GitHub issue/PR prose. Exempt: mechanical fixes',
...
function buildPlanPrompt(strategy, tactic, gather) {
  return [
    ...
    '  typechecks, builds) go in fenced ```verify blocks; invoke an app unit suite',
    '  as `npx vitest run --project <app> --root <repo_root>` (never --root <app>).',
