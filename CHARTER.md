...
**Review trigger:** when projected runway drops below the estimated time-to-revenue, reassess before continuing at the current spend. The default response is to *slow down*, not pay more — the construction-tool-vs-runtime distinction means reducing iteration velocity costs time but breaks nothing already built. Sustaining or raising spend through a breached threshold is a deliberate choice, justified only by a credible revenue trajectory, never the default.
