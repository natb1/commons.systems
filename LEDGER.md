...
Transient. Every entry names the disposition, a node on the `disposition` ref, that will
...
| L15 | `/align` is materialized from the ratified schema | stubbed as a shim | author | `.claude/skills/align/SKILL.md` on the `greenfield` ref |
...
| L30 | evaluation doctrine; shims apply by default | stubbed | author (legacy record) | `disposition-graph/evaluation`; `.claude/rules/evaluation.md` on `greenfield` |
| L31 | `CLAUDE.md` is justified and materialized by disposition | stubbed | author | `disposition-graph/session-context`; `projection` |
| L32 | ratification is the dialectic's outcome; the ratify script is liquidated | stubbed | author | `disposition-graph/authority`, `growth` |
...
`disposition`, no PR. Ratified stamps are written by the alignment session in
the author's name after the author's ruling in the interview (L32; the
ratify command of round zero is liquidated). Tooling shim: the
package imports `yaml` from an ancestor
...
author's framing; the author rules. Six readings are stubbed on the `disposition` ref as
the first instances.
...
tool is the instrument of one of them: validator (node, and the stamp check
for authority), lander (persistence), projector and browser (projection),
rank and frontier derivation (attention, work-loop), the reconciler tick
(work-loop).
...
authority, growth, persistence, readings, review, evaluation, and attention,
and lives on the `greenfield` ref at `.claude/skills/align/SKILL.md`. Stubbed
2026-09-02 as a hand-written shim, used for the first ratification. Author:
"For bootstrapping we are implementing the alignment skill from scratch, but
reference the incumbent for key principles that should be adopted - first for
shimming/stubbing, then for materialized skill before bootstrap exit. We will
need this shim to ratify the first node." The survey of the incumbent record
is `bootstrap/align-survey.md`. Adopted from the incumbent, as principles:
the record as the sole carrier; the interview as the audit; the two interview
types with the record at origin as the fixed object, the author's account
first, compulsion by argument only; the three exits; the recommendation first
carrying its authority class, a boldness assessment, and genuine
alternatives; context delivered where the author reads it; deferral as a
stamp, never a side list; a shortlist never disposes and the dialectic is
never delegated; one question per node; whole-node amendment; the coverage
walk; the mechanical floor; doctrine currency, with shims applied by default
(the author's legacy rulings [9], [13]); landing location never asked
(legacy [4]); fable as the skill's default model (legacy [8]). Rejected with
its mechanics: GitHub, tactics, phases, the router and its gates, born-parked
review items, the placement gates, the curriculum, and the skill's own text
as authority (survey section D). Liquidation: the projector materializes the
skill from the ratified nodes and the hand-written file is deleted.
...
**Stub.** `bootstrap/model-proposal.html` on the `greenfield` ref is the onboarding page
...
**Content.** Round zero's working definitions, in `CLAUDE.md` on the `greenfield` ref:
...
**Content.** Recorded in `CLAUDE.md` on the `greenfield` ref, which every
session and subagent rooted in that worktree loads. Session memory was evaluated and
...
frontier (L11). The reader, validator, and projector are instruments of
`node`, `authority`, and `projection`; the ratify command of round zero is
liquidated (L32).
...
**Source.** Author, in the legacy bootstrap record (steering ledger [7],
2026-09-02): "Remember greenfield evaluation rules: nothing is sacred, esp.
the incumbent implementation. Ratified doctrine only needs an interview to
overrule, other dispositions don't even require that. Consider from the
perspective of best greenfield judgement and also reference to tradition."
Ledger [9]: "Bootstrap shims must include /align shims during bootstrapping
so that this kind of alignment session is as close to greenfield as
possible." Ledger [13]: "the alignment skill must use any alignment shims by
default, and only bypass shims if prompted." Given again to this bootstrap
2026-09-02: "reference the incumbent for key principles that should be
adopted".

**Content.** Stubbed in `disposition-graph/evaluation` (global tier, under
`growth`) and projected by hand to `.claude/rules/evaluation.md` on
`greenfield`, a shim until rules are materialized from global-tier nodes
(L05). The legacy rule file `.claude/rules/greenfield-evaluation.md` on
`main` and the ratified clarifications it projects (2026-08-31 and
2026-09-01 on the legacy node `strategy-explicit-intent`) are the evidence.
...
the technical vocabulary - refer to tradition. It's hard to keep track of
which is which."

**Content.** Defined in `disposition-graph/growth`, with two readings under
it. Periagogic (the incumbent's type b): the record is authoritative and the
author is turned back to it; Plato, Republic VII 518c to d, the turning of
the soul, the word the incumbent skill already used for this conduct.
Maieutic (the incumbent's type a): the answer lives in the author and the AI
draws it out with visible, refusable drafts; Plato, Theaetetus 148e to 151d,
Socrates as midwife, with the test of the offspring at 151e as the origin of
the refusable draft. A sitting's comprehension stage is periagogic and its
intention stage maieutic. The letters are retired from the shim.

## L34 `/align <node id>` is the two-stage review sitting

**Source.** Author, 2026-09-02: "`/align <node_id>` is expected to behave
similarly to the dialectic for reading/curriculum review in the incumbent
graph/implementation. Dialectic to make sure I understand what's there, and
dialectic to make sure my intentions are understood."

**Content.** Recorded in `disposition-graph/growth` and the shim. Adopted
from the incumbent review sittings (the author's direction of 2026-07-13,
"First test my comprehension, then interview to ratify", and the reading
exercise's movements: reading, comprehension dialectic, integration,
follow-up curriculum, ratification, on the legacy node
`strategy-graph-review-curriculum`): stage one, comprehension, in which the
author articulates what the record and its readings say before the AI's
account enters as counterpoint, probes cite the text by locus, and no
verdict is in play; stage two, intention, in which what the author means
and intends to bind is elicited and tested, and the ruling is taken. Each
...
**Source.** Author, 2026-09-02: "doctrine on usage: `/align <disposition>` is
the incumbant usage and is maintained. New usage `/align <node_id>` applied
the same principles to ratify or review ratification of a node."

**Content.** Recorded in `disposition-graph/growth` and in the `/align`
shim: given a disposition in the author's words, the skill records or
revises the node that answers it; given a node id, it runs the same
dialectic to ratify the node or to review its ratification. Round zero's
first use of the second form is the ratification interview for `purpose`.

## L32 ratification is the dialectic's outcome; the ratify script is liquidated

**Source.** Author, 2026-09-02: "What principles from the deferred/shimmed
alignment skill can be applied to ratification of the existing node.
Ratification is not a rubber stamp. I don't see the function of a
ratification script and it can probably be liquidated with updated
disposition/doctrine. The alignment dialectic is specifically designed to
ensure two way alignment on a) the proposal from the AI and b) the
intention from the author."

**Content.** Amended in `disposition-graph/authority` and `growth`, and in
the `/align` shim, step 5. Ratification is the outcome of the dialectic:
the interview type stated; the author's own account before the AI's; the
three classes of finding surfaced; the evaluation run twice; the draft
tested against the record it joins; every decision mapped to a field; the
recommendation carrying its authority class, a boldness assessment, and the
alternatives; the three exits open. When the author rules, the session
writes the stamp in the author's name and quotes the ruling with its date in
the node. The ratify command guaranteed nothing (it stamped under the same
version-control identity every session commits with) and is deleted from
`packages/disposition` with its tests; the validator keeps the stamp check.
Rejected: the round-zero design in which the author's running of a command
was the act (L04, amended).

## L31 `CLAUDE.md` is justified and materialized by disposition

**Source.** Author, 2026-09-02: "CLAUDE.md must be justified/materialized by
disposition just like rules and CLAUDE.local.md - it may exist, but evaluate
how it interacts with disposition for materialization, and write that
disposition. Does CLAUDE.md serve any function not served by (materialized)
rules?"

**Content.** Stubbed in `disposition-graph/session-context` under
`projection`, global tier. The evaluation: rules and `CLAUDE.md` are both
always-loaded; rules are project-root scoped and may be path-scoped,
`CLAUDE.md` is directory-scoped up the tree and may import files. The one
function `CLAUDE.md` serves that rules do not is orientation, the surface an
arriving session reads first; it is kept for that and projected from
`purpose` and `projection`, stating no rule of its own. The bootstrap
`CLAUDE.md` on `greenfield` is a declared shim: its token-efficiency rule and
loop liquidate into `bootstrap` and the skill (L21, L23), its review recipe
into the `review` instrument (L29), its environment traps into evidence on
the nodes they qualify, its round log and decisions list into this ledger
and git history. Anything left that no node projects is a prune-by-default
proposal (L11).
...
**Amendment, 2026-09-02 (author).** "The code review is expensive. You can
let the current one complete, but during bootstrap code review is only
required after ratification. This is analogous to the recorded greenfield
doctrine in the legacy graph that the reconciliation frontier should first
validate functionality, and only spend cycles on cross-cutting
non-functional validation after that." Recorded in `review`: during
bootstrap a landing is reviewed once the disposition it materializes is
ratified, and everything is reviewed before exit; until then tests and use
suffice. Round one of the tooling review completed (426 s, 8 findings);
its fixes are verified by tests; round two is owed after ratification of
the nodes the tooling instruments.
