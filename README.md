...
5. **Topic-category × phase ladder.** Topic categories nest outside the phase ladder. Categories, highest first: `priority` → `bug` → `testing infrastructure` → `dispatch` → `other`. `priority` is a human-applied escalation marker; the selector exhausts one category's full ladder before moving to the next. Within each category the phase ladder is, highest first: `security` → `review` → `code-review` → `qa` → `implement`.
