...
const audioHosting = firebaseConfig.hosting.find(
  (h: { target: string }) => h.target === "audio",
);
if (!audioHosting) {
  throw new Error("firebase.json missing hosting config with target 'audio'");
}
const headers: { source: string; headers: { key: string; value: string }[] }[] =
  audioHosting.headers;
if (!headers || headers.length === 0) {
  throw new Error("audio hosting config has no header rules");
}

describe("audio security headers", () => {
