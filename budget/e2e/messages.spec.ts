...
test.describe("messages", () => {
  ...
  test("home page displays seeded messages", async ({ page }) => {
    ...
    await expect(messages.nth(0)).toContainText("Welcome to budget");
    await expect(messages.nth(1)).toContainText("Hello from Budget");
  });
  ...
  test("messages appear in chronological order", async ({ page }) => {
    ...
    expect(texts[0]).toContain("Welcome to budget");
    expect(texts[1]).toContain("Hello from Budget");
  });
