...
// use-app-state.ts. "/" (Budgets), /transactions (Transactions), and /rules
// (Rules) are real React pages; accounts + reconcile are still legacy string
// renderers wrapped in <LegacyRoute>.
...
// "/" (Budgets), /transactions, and /rules are real React pages, not
// <LegacyRoute>s; they live outside ROUTES but are still known paths for routing
// + Nav `current`. Budgets is also the unknown-path fallback (it was ROUTES[0]
// in the legacy router's `?? ROUTES[0]` fall-through).
const BUDGETS_PATH = "/";
...
// ("/"), /transactions, the LegacyRoute paths (accounts, reconcile), then
// /rules last (its original slot).
const KNOWN_PATHS = [BUDGETS_PATH, TRANSACTIONS_PATH, ...ROUTES.map((r) => r.path), RULES_PATH];
...
export function App() {
  ...
  // /rules, not a LegacyRoute) render the React Budgets page — it is the JSX
  // fallback below.
  const currentPath = isTransactions
    ? TRANSACTIONS_PATH
    : isRules
      ? RULES_PATH
      : (legacyRoute?.path ?? BUDGETS_PATH);
  ...
  return (
    ...
    >
      {/* Keyed by path AND navEpoch: a fresh instance per link navigation
          ...
          "/" (Budgets), /transactions (Transactions), and /rules (Rules) are
          real React pages — same keying so a same-path data transition re-mounts
          them and re-resolves; renderOptions() is read at mount and also calls
          setActiveDataSource for the route, matching how LegacyRoute is fed. The
          remaining routes (accounts, reconcile) stay legacy.
