...
const budgetHosting = firebaseConfig.hosting.find(
  (h: { target: string }) => h.target === "budget",
);
if (!budgetHosting) {
  throw new Error("firebase.json missing hosting config with target 'budget'");
}
const headers: { source: string; headers: { key: string; value: string }[] }[] =
  budgetHosting.headers;
if (!headers || headers.length === 0) {
  throw new Error("budget hosting config has no header rules");
}

describe("budget security headers", () => {
