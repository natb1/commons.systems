...
import type { CategoryVariance } from "../../src/balance";

vi.mock("firebase/firestore", () => timestampMockFactory());

import { buildWaterfallBars, renderVarianceWaterfall } from "../../src/pages/budgets-waterfall-chart";

function cat(category: string, avgWeekly: number, isOther = false): CategoryVariance {
  return {
    category,
    avgWeekly,
    percentOfActual: 0,
...
describe("buildWaterfallBars", () => {
  ...
  it("running total reflects multiple categories in order", () => {
    const bars = buildWaterfallBars({
      weeklyAllowance: 150,
      categories: [cat("A", 100), cat("B", 40), cat("Other", 10, true)],
      window: 12,
    });
    // Allowance + 3 categories + Actual
  ...
  it("handles zero-category case", () => {
    ...
    expect(bars[0].label).toBe("Allowance");
    expect(bars[1]).toMatchObject({ label: "Actual", y1: 0, y2: 0 });
  });
});
