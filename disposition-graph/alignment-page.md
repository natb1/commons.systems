...
The eight questions this answer no longer settles alone. The decomposition of
...
takes in prose: `authors-words-on-the-page`, where the author's own words
appear; `how-a-fact-is-headed`; `the-account-on-the-page`;
`vocabulary-option-summary`; `what-an-option-row-carries`;
`when-the-kickback-feedback-shows`; `where-a-change-request-goes`; and
`which-facts-are-listed`, which facts the page lists on a node. A ninth,
`input-for-an-unfinished-movement`, is answered by the author's rule of
2026-09-06, which this answer now states; it stands until the author rules its
prune and is named in `depends` while it does. All eight stand at the
periagogic stage and all eight are named in `depends`, so a ruling here waits
on them. Where a clause of this answer touches one of those decisions it is
marked above as standing only until that node rules; the clauses that remain
are this node's own, the three columns, what the rail carries, what the page
takes from the author, and how a ruling is transported.
