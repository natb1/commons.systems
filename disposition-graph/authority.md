...
- `## Facts` carries a `#### authority-derived` subsection for the option named by `stands` (brief line 1282 renders its stored prose in place of the answer's first sentences). See the cross-node `contradiction` finding naming nine nodes.

Strongest counter-argument (strong): This answer carries, in one unbroken paragraph on one answer fact, at least a dozen separable decisions: what ratification is, what delegated and deferred confer, how a ratified authority fact blocks an ancestor's delegation, how ties between ancestors resolve, what doctrine is, what `proposal` means, and what the AI may do within scope. `commons.systems/disposition-graph/delegation-bounds-and-sizing` argues that "a ruling is recorded per fact, and `delegation` has one answer fact, so there is no act by which the author ratifies part of it", and that argument bears harder here than on the node that raised it. Ratifying this node is one act over a dozen decisions, and the record has just decided, one node away, that such an act is not recordable.

The session's reply: Taken, and it is the strongest thing the survey found about this node. The argument of `delegation-bounds-and-sizing` does bear harder here, and the survey's own `decomposition` finding says so in general terms; what this node needs before it is put to the author is the same examination — which of the dozen decisions in the paragraph the author might rule on separately, and which of them belong on nodes of their own. The session records that as owed and does not perform the split, which is a design act and not a reply. The standing-option subsection is taken with the other eight.
...
Recorded as an option on this node's answer fact: `no-census-anywhere-in-a-node` (source review, 2026-09-05).
...
Also named: commons.systems/disposition-graph/delegation, commons.systems/disposition-graph/dialogue, commons.systems/disposition-graph/evaluation, commons.systems/disposition-graph/readings, commons.systems/disposition-graph/recording, commons.systems/disposition-graph/rejected, commons.systems/disposition-graph/unanswered, commons.systems/disposition-graph/viable-options, commons.systems/disposition-graph/alignment-page.
...
### Clean-context re-reading, 2026-09-07, of 15694b67
...
- `## Facts`, `##### answer`, opening reason sentence: "The recommendation is `authority-derived`, at moderate boldness." This is now false: the diff changed the fact's `recommends` from `authority-derived` to `the-ruling-is-quoted-in-the-record-and-referenced-by-the-option` (frontmatter hunk: `-    recommends: authority-derived` / `+    recommends: the-ruling-is-quoted-in-the-record-and-referenced-by-the-option`), while leaving this prose sentence — which is what the reason paragraph the fact's moderate boldness rests on opens with — naming the old option as 'the recommendation'. A reader of the prose alone is told the wrong option is recommended. Suggested edit: open with 'The recommendation is `the-ruling-is-quoted-in-the-record-and-referenced-by-the-option`, at moderate boldness, amending `authority-derived`, which stands,' and say why the amendment is now recommended over the option that stands.
- Same paragraph, closing sentences: "Three options stay viable beside it: `stamped-classes` ... `ceiling-moves-here` ... and `escalate-toward-ratified` ... The rest are passed over with their reasons on their rows." This is stale after the same diff, which added four further options with no `status: passed` and no ruling — `no-census-in-a-standing-answer`, `authority-per-clause-by-a-child-node`, `a-conferred-class-acts-during-bootstrap`, and `no-census-anywhere-in-a-node` (all added in the frontmatter hunk alongside the new recommended option). None of the four carries a passed-over reason on its row; each is recorded, per its own subsection, as a live option the author has not ruled. 'The rest are passed over' is therefore false of at least four of the ten remaining options. Suggested edit: update the count and list to name all options that stay viable (seven, not three) and drop or correct the claim that the rest are passed over.
- The survey's `contradiction` finding of 2026-09-05 on the nine nodes carrying a stored `#### <option>` subsection for the option their answer fact names in `stands` ("the encoding rule is that the standing option omits its subsection ... The nine ... `authority` (`authority-derived`)") is not addressed by this amendment. The node as it now stands still carries `#### authority-derived` under `##### answer` with its own prose, even though `authority-derived` is the option named by `stands` and `dialogue`'s own answer says a standing option's subsection 'needs none, since its text is the answer'. The amendment edited that subsection's text (the periagogic-clause fix, F12) but did not remove it, so `authority` remains one of the nine nodes the survey found violating the standing rule; the account's reply to the survey finding does not mention this node's own copy of the defect. Since the finding is dated on or after this node's last review and names `authority` directly, it is owed an answer here: either delete the `#### authority-derived` subsection (the text is not lost — `## Answer` and the account carry it) or state why this node is deferring the fix to the pending ruling on `dialogue`.

On the facts and what they recommend: The diff moves the answer fact's `recommends` from `authority-derived` to the newly added option `the-ruling-is-quoted-in-the-record-and-referenced-by-the-option` (still moderate boldness, `stands` unchanged at `authority-derived`), and correspondingly a `## Recommendation` fence now appears carrying the amended node text (the ledger/quotes formulation of the validity clause) in place of the previous no-fence state where recommends equalled stands; the fence's `defines`, `depends`, `## Answer` and `## Rationale` all match the answer text elsewhere in the diff. The authority fact (ratified, moderate) is untouched by this diff.

On the viability of the options: The diff adds four new unflagged options (no-census-in-a-standing-answer, authority-per-clause-by-a-child-node, a-conferred-class-acts-during-bootstrap, no-census-anywhere-in-a-node) that are correctly left viable and unpassed, consistent with the record's rule that a candidate never silently leaves the list; but the fact's own reason paragraph was not updated to reflect them and still asserts only three options stay viable with everything else passed over, which is now inaccurate (finding 2).

Strongest counter-argument (moderate): The amendment answers all twelve of the previous reading's findings and correctly records the census-contradiction survey finding as an option, but in doing the mechanical work of adding the new recommended option it left the fact's own reason paragraph internally inconsistent with the frontmatter it sits beside — naming the superseded option as 'the recommendation' and undercounting the viable options — which is exactly the kind of locally-verifiable factual error about the record that the previous reading spent its findings correcting elsewhere on this same node, now reintroduced one layer over.

### Repaired after the second re-reading, 2026-09-07

The two residues repaired on the main thread: the clause counting three viable options with the rest passed over is struck, the options being what their subsections say; and the subsection `authority-derived` carried under the facts while it is the option that stands is removed, its prose being the first sentences of `## Answer` as the encoding of the day requires, which is the survey's contradiction finding of 2026-09-05 on this node discharged. Pin-neutral; the node returns to review for the re-reading a kickback owes.

### Clean-context re-reading, 2026-09-07, of 8d217d7d
...
- `## Facts`, `##### answer`, option `authority-derived` (the option named by `stands`): the diff's only two hunks are the frontmatter review block and the fact's opening reason paragraph (`@@ -168,7 +168,9 @@ force.`, splitting `-The recommendation is \`authority-derived\`...` into the new lead paragraph plus the retained old one); neither hunk touches this option's own subsection. The node as it now stands still carries, under `\`authority-derived\` — source author, ref 2026-09-04 — stands (its text is the '## Answer' above)`, its own prose beginning "The class is read off the rulings recorded on the node's facts and no stamp is written beside them: ratified where the answer fact carries a ruling, delegated or deferred where the authority fact does, unanswered where none does, and a class the AI writes for itself is not a grant," duplicating `## Answer`, contrary to the record's own encoding rule quoted in this brief: "The option named by `stands` omits its subsection, because its sentence is the first sentences of `## Answer`." This is exactly the previous reading's third finding (citing the survey's `contradiction` finding of 2026-09-05, which names `authority` (`authority-derived`) directly among the nine violating nodes), and the amendment leaves it completely unaddressed — it neither deletes the subsection nor states why the fix is deferred to `dialogue`'s pending ruling. Suggested edit: delete the `#### authority-derived` subsection's body (the text already stands in `## Answer` and in the fact's own reason paragraph), or add a sentence explicitly deferring the fix to `dialogue`.
- `## Facts`, `##### answer`, second paragraph, retained verbatim from before this round's fix: "Three options stay viable beside it: `stamped-classes`, the mark stored on the node if the author prefers it; `ceiling-moves-here`, which is not dominated; and `escalate-toward-ratified`, whose rule left this answer on 2026-09-05 for the node `class-recommendation`, where it is the recommendation, and which stays on the list because no candidate leaves it, shown as moved rather than passed over once the `adopted-is-a-status` option on the viable-options node is ruled, which `depends` names. The rest are passed over with their reasons on their rows." This is the exact sentence the previous reading's second finding quoted as false, since four further options (`no-census-in-a-standing-answer`, `authority-per-clause-by-a-child-node`, `a-conferred-class-acts-during-bootstrap`, `no-census-anywhere-in-a-node`) carry no `status: passed` and are not among the three named. The amendment's fix is to prepend a new paragraph stating the newer options "stay viable beside it, each as its subsection says, and none is passed over by this amendment," and to frame the old paragraph as history ("What follows is the reason `authority-derived` was recommended on, which the amendment carries") — but it does not edit the sentence itself, which is still written in the present tense and, read on its own, continues to assert that only three options stay viable and the rest are passed over, which was never true from the point the four newer options entered this fact's list. The previous reading's suggested edit was explicit — "drop or correct the claim that the rest are passed over" — and the amendment did neither, so this finding is only partially answered, not closed. Suggested edit: rewrite the retained sentence to drop the closing clause, or recast it in the past tense as a snapshot of the option list before 2026-09-05's additions, so it no longer reads as a present, false claim.

On the facts and what they recommend: The diff splits the answer fact's reason paragraph into two: a new lead paragraph correctly names `the-ruling-is-quoted-in-the-record-and-referenced-by-the-option` as the current recommendation (closing the previous reading's first finding) and correctly states that the options recorded since 2026-09-05 stay viable and none is passed over by this amendment. The retained second paragraph, unedited apart from being re-labelled as the reason `authority-derived` was recommended, still asserts in the present tense that only three options stay viable and the rest are passed over — false of the four newer options, and only partially answering the second finding. `stands` (`authority-derived`), the authority fact, and the `## Recommendation` fence's content are otherwise untouched by this diff.

On the viability of the options: The diff itself changes no option's status, and the four post-2026-09-05 options remain correctly unflagged as viable rather than silently dropped. But the retained sentence in the answer fact's own prose still misstates the count and falsely claims 'the rest are passed over,' and the `#### authority-derived` subsection under the `stands` option — the survey's `contradiction` finding and the previous reading's third finding, both naming `authority` directly — is untouched by this diff, so that duplication of `## Answer` persists unaddressed.
