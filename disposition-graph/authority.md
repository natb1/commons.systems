...
The recommendation is `authority-derived`, at moderate boldness. The derivation of the class from the rulings, the grant that does not expire, and the proposal as a state of a ratified node are the author's words of 2026-09-04; the strike of the 2026-09-03 expiry against the words of that day, the list of a move's origins, the return of a proposal to the review stage, the escalation test the AI applies when it recommends a class, promoted here from the alignment skill, a declared shim, and never a disposition of the author's, and the citation of what acts during bootstrap are the AI's. Three options stay viable beside it: `stamped-classes`, the mark stored on the node if the author prefers it; `ceiling-moves-here`, which is not dominated; and `escalate-toward-ratified`, whose rule the answer has adopted and which stays on the list because no candidate leaves it, shown as adopted rather than passed over once the `adopted-is-a-status` option on the viable-options node is ruled, which `depends` names. The rest are passed over with their reasons on their rows.
