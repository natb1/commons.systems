...
### What a design unit returns, 2026-09-07

The author, 2026-09-04, quoted under `## Disposition`, asked that complex dispositions be broken into units and integrated by the main thread, "from the perpective of token/context efficiency and also management of AI attention". The author, 2026-09-07: "also consider optimzations to the dialogue workflow"; and "record the recommended optimizations as dispositions, progress them up to confirmation".

The measurement, at implementation commit `87e4b24e`, on the design units of 2026-09-06 and 2026-09-07 in `tmp/align/`: `design-turn-form.md` 16,432 bytes, `design-change-request.md` 28,615, `design-authors-words-2.md` 36,674, `design-option-row.md` 37,109, `design-worc-2.md` 39,031, `design-authors-words.md` 39,315, `design-scope-rule.md` 40,397, `design-growth-2.md` 44,306; 281,879 bytes in all. Every one of them redrew whole the sections it changed; `design-growth-2.md` alone returned any locus as an old and a new text, in its `## rationale-standing-edits` section, which is the shape this clause makes the rule.

What moved. The recommendation moves from `seams-and-split-review` to `a-unit-returns-the-amendment`, which is that text with the pairs clause added to the paragraph on what every unit returns, and one addition to the rationale's costs. The five seams, the order the units run in, the models on each, and the division of the review by its object are unchanged.

What stays viable. Every option keeps its row. `seams-and-split-review` is the text this option amends and is the author's fallback if the clause is refused; `pre-review-under-the-batch`, `decomposition-before-minting` and `no-carrier-and-the-questions-are-progressed` are untouched, and the last of those is still the author's own words of 2026-09-05 waiting on a ruling.
