...
### The falsified census clause struck, 2026-09-09

The answer's measurement of what the record owes closed with "and not one node
carries a single ruling, so nothing anywhere in this record is confirmed today
and every projection that shows a text in the confirmed place is showing the
AI's draft". The clause is struck at all four loci it stood in: the base option
`the-survey-block-carries-what-the-next-survey-selects-on`, which holds the
content whole, and the three options that chain from it by diff and carried the
clause as a context line, `a-pin-covers-what-binds-the-node`,
`probes-are-a-part-that-outlives-the-dialogue`, and the recommended
`an-option-accumulates-per-expert`.

What falsified it: three nodes now carry a ruling on their authority fact,
`expert-instructions`, `movements` and `round-termination`, each a confirm on
the `deferred` option dated 2026-09-08 and referencing `words/2026-09-08/39`.
`expert-identity` carries no ruling of its own and takes a conferred class from
`movements`' delegation, which is the walk `authority`'s answer describes and
not a fourth ruling. The frontier survey of 2026-09-09 found the clause and
named it a supersession; that finding and the session's reply are recorded
above.

The clause is struck rather than restated with a new count, which was the
survey's own proposal. What it was tracking survives as a rule and not a number:
`authority`'s answer now holds that a node's answer is the content of the option
the author confirmed, that where none is confirmed the node has no answer, and
that the reader's supplying the recommendation in that place is a defect of the
implementation against the rule. That is the same fact stated so that it cannot
go stale, and it is stated on the node where what acts is said.

The rest of the sentence stands. Its numbers are pinned to graph commit
`93644144` and none of them has been shown false, so striking them would be the
AI applying `authority`'s unruled option `no-census-in-a-standing-answer` to
loci the record has not measured as wrong, which is a ruling the author has not
made. The sentence remains a census in a standing answer, and it is named here
as evidence for that option and for the wider `no-census-anywhere-in-a-node`,
both of which stay unconfirmed and unpassed on `authority`'s answer fact.

### The fallback clause reworded to the authority node's rule, 2026-09-09

The base option `the-survey-block-carries-what-the-next-survey-selects-on` held in one sentence that a node's answer is the resolved content of the option labelled confirmed, "where none is, of the option the answer fact recommends", and that a node no ruling reaches has no answer; the middle clause contradicts the last, and it contradicts `authority`, whose recommended text holds that where no option on the answer fact is confirmed the node has no answer, and that the option the fact recommends is a recommendation and never a fallback answer standing in for one. The middle clause is struck. The sentence now says that where none is confirmed the node has no answer, and that the recommended option's resolved content is what a projection carries for such a node, as a draft and never as its answer. The sentence stands in the chain's base and no dependent hunk carries it as context, so one locus was enough, and the recommended option `an-option-accumulates-per-expert` resolves as before with the one sentence changed. The implementation landed on `greenfield` at 19a30a90 the same day: the reader's `answerText` is the confirmed option's resolved content alone and null where none is confirmed, and the text a projection renders where nothing is confirmed is `carriedText`, named as what is carried and not as the answer; every projection's output was measured unchanged by the split.
