...
- Node, whole: no '## Disposition' and no ruling quoted of any date. The Rationale rests the answer on 'The author's rulings in the legacy bootstrap of 2026-08-31 to 2026-09-02, given again to this bootstrap', with the legacy rule projection as evidence. Authority: 'a ratified stamp whose ruling is not in the record is invalid'; legacy: legacy nodes are evidence and never authority. This node is projected to .claude/rules/evaluation.md and binds every session today.
- Answer: 'An overrule of a draft at the ruling stage sends the node back to the review stage, and the frontier and the alignment page show the draft as changed since its review.' Verified true: renderFrontier prints ', draft changed since the review' and the alignment renderer emits a 'draft changed since the review' pill. The previous review's finding is answered and the mechanism works.
- Answer: 'every tradition surfaced is recorded as a reading with the resolution it informed', while this node's own Rationale carries a prose tradition list (the Pareto frontier, Chesterton's fence). Fourteen rationales do the same. The rule is unmet by the node that states it.
- Frontmatter 'tier: global': a kickback or overrule here changes what sessions are running under today, which the Proposal does not say.

On the three facts: The frontmatter recommendation (ratified, moderate) states one class and one value; the prose Facts line is the generic template stating two. The facts should say that this text is live doctrine-by-shim today, projected into .claude/rules/, so the ruling is not prospective.
...
The session's reply: Validated. The ruling a ratified stamp requires is given at the sitting and quoted then; the legacy rules are evidence, which the rationale says. The rule that traditions are recorded as readings is unmet by this node's own rationale until the readings rule is ruled. This text is live doctrine by shim, projected into the rules directory, so the ruling is not prospective. On the counter-argument, that nothing is sacred licenses re-deciding the graph between sittings: the brake at every stage is the author's words on the node, the batch review, and the hash the frontier flags; the author rules whether that suffices. Stage ruling.
...
Also named: commons.systems/disposition-graph/authority, commons.systems/disposition-graph/delegation, commons.systems/disposition-graph/dialogue, commons.systems/disposition-graph/readings, commons.systems/disposition-graph/recording, commons.systems/disposition-graph/rejected, commons.systems/disposition-graph/unanswered, commons.systems/disposition-graph/viable-options, commons.systems/disposition-graph/alignment-page.
...
Also named: commons.systems/disposition-graph/quotes, commons.systems/disposition-graph/persistence, commons.systems/disposition-graph/review, commons.systems/disposition-graph/validation-order, commons.systems/disposition-graph/legacy, commons.systems/disposition-graph/recording.
...
Also named: commons.systems/disposition-graph/stub-traditions, commons.systems/disposition-graph/readings, commons.systems/disposition-graph/codd-update-anomaly, commons.systems/disposition-graph/instruments, commons.systems/disposition-graph/materialization, commons.systems/disposition-graph/validation-order.
...
The survey of 2026-09-05 raised a `stale-recommendation` finding naming
thirty-four nodes, this one among them, and the apply discarded it because one
node it named, `delegation-bounds-and-sizing`, had moved after the survey read
it. The finding was validated at its loci on the main thread and is not
withdrawn; it is recorded here, on the node the survey names as the survivor for
the general rule, so that nothing it found is lost with the mechanism that
dropped it. The stages it named were not set by hand: a discarded finding's
nodes are judged again by the next survey, and a session standing in for the
instrument is what the instrument exists to prevent.
