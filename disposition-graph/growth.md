...
By a loop of three moves. Propose: the AI writes a node, or an amendment, in the record with no more authority than it holds. Project: the node's page in the graph browser is rendered, because every node has a documentation projection and the page is what the author reads. Ratify or steer: the author ratifies, which stamps the node, or steers, and the steer enters the node's rationale as a rejected alternative or an amendment before the page is rendered again. During bootstrap the loop runs by hand in the bootstrap session; afterwards it is the `/align` skill, materialized from this node and its siblings. Legacy nodes are cited as evidence when a question needs them and never imported.
