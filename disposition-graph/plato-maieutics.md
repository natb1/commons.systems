...
- Facts line: 'ratified if the author confirms' against readings' rule that ratified means the author has read the primary text; the Rationale says the reading is 'deferred until the author reads them'. Delegated is what a confirmation confers.
- Rationale: 'The divergence to keep in view: the AI is not barren; it drafts, and the draft can lead.' The frontmatter says 'relation: adopted'. Readings provides 'diverged' for exactly this, and evaluation requires a divergence to be recorded; a divergence named in prose under an adopted relation is the unrecorded conflict evaluation calls 'a frontier item deferred to neither side'. Suggested edit: set relation to diverged, or say why the divergence leaves the relation adopted.
- Frontmatter carries source and relation but names no tradition, and no traditions graph exists in disposition.yaml; readings' recommended answer requires the reading to name the tradition it reads.
- The loci in the answer ('150c to d', '151e') differ from the range in 'source' and from the locus growth's answer gives for the same conduct ('Theaetetus 148e to 151d'), so a reader checking the ground of the maieutic conduct is given two citations. Growth's periagogic conduct requires probes that 'cite the text by locus'; the loci must agree.

On the three facts: Generic template with the wrong class for a reading. Boldness should be moderate: the adoption is straightforward, but the claim that the AI's visible draft plays the role of the midwife's test is the AI's own analogy and is what the counter-argument attacks.
