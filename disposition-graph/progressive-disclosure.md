...
- The `## Account` of this node carries the formula "as every reading on the record recommends" at two loci, and the live `### authority` prose no longer does: the node has already corrected its own text. Context for the cross-node `contradiction` finding, not a defect on this node.

Strongest counter-argument (moderate): The tradition's criterion is frequency and importance of use across readers, and a page with one reader cannot measure it, so the divergence recorded on the fold may be a divergence in name only: applied to one reader the tradition has nothing to say, and recording a divergence where the source is silent overstates what the author decided against. The same objection reaches `software-factories`, which records a divergence where its tradition is likewise silent, so this is the record's pattern and not this node's slip — a reading required to produce an adopted limb and a diverged limb will manufacture the second. The substitute criterion, boldness, is the AI's own measure of its own warrant, so the page folds by the AI's self-assessment and calls it a principled split.
