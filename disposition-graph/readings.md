...
- The defect the launching thread asked to be checked is present and is worse than a missing omission. disposition/disposition-graph/readings.md line 131 carries `#### relation-per-option` inside `## Facts` — a subsection for the option named by `stands` and by `recommends`, which the encoding rule forbids: "The option named by `stands` omits its subsection, because its sentence is the first sentences of `## Answer`". The stored prose does not contain the answer's first sentences; it opens "A reading stays a node under one node it bears on, with its own class, and its relation attaches to the options of the fact it bears on rather than to the answer", which is a description of what the option changed, not the answer. So every projection that reads the standing option's subsection renders a delta where the standing sentence belongs. The remedy is to delete the subsection, not to rewrite it, since `## Answer` already carries the text.
- The frontmatter `defines:` at disposition/disposition-graph/readings.md line 60 still lists `chosen over`, while the `## Answer` says of that term "it is derived and never stored; the option's readings are the derived inverse of what the readings bear on". A node cannot define a term its own answer says the record does not carry as a stored thing; either the definition says it names a derivation, or it comes off the list. The reader at packages/disposition/read.mjs:107 carries `RELATIONS = ['adopted','diverged']` and no third, which confirms the answer and not the frontmatter.
- The authority fact's `against` (line 47, repeated at line 155) reads "every reading on the record recommends delegated for itself". Measured at this commit: 59 nodes carry `form: reading`, 57 recommend `delegated`, `srs-introduction` recommends `deferred`, and `npm-committed-lockfile` carries an authority fact with options and no `recommends` at all. The claim is false as stated, and it is the same false census the cross-node `contradiction` finding records on forty-six other nodes.

Strongest counter-argument (strong): The per-option relation binds a tradition's verdict to option names, which are the AI's handles and move under it as options are added, renamed and composed, so the option the author confirms can show no tradition beside it while a rival shows one. The answer concedes the guard against that — the duty to re-point a reading when a recommendation moves — is "stated here and unchecked, unmet across the record at this commit". So the node asks for ratification of a rule the record is already in breach of, with no instrument, and with the check that would find the breach recorded on another node as an unruled option. A relation per holding, or per node, costs files and buys a handle that does not move.

The session's reply: Taken on all three findings and on the counter-argument. The `#### relation-per-option` subsection is deleted rather than rewritten, since `## Answer` carries the text; `chosen over` comes off `defines`, because the answer says it is derived and never stored and `RELATIONS` in the reader carries no third value; and the `against` census is struck for the rule it was counting, with the two counterexamples left standing as the finding proposes. On the option-name instability the session concedes the guard is unmet across the record at this commit and does not move the recommendation, because a relation per holding buys a stable handle at the price of the thing the per-option encoding was for, which is showing the author a tradition beside the option it bears on.
...
Also named: commons.systems/disposition-graph/authority, commons.systems/disposition-graph/delegation, commons.systems/disposition-graph/dialogue, commons.systems/disposition-graph/evaluation, commons.systems/disposition-graph/recording, commons.systems/disposition-graph/rejected, commons.systems/disposition-graph/unanswered, commons.systems/disposition-graph/viable-options, commons.systems/disposition-graph/alignment-page.
...
### Clean-context re-reading, 2026-09-07, of 9849bdfe
...
- ## Facts, ##### answer: the summary line reads "recommends the-relation-is-projected-onto-the-option-as-one-of-three (moderate) of relation-on-the-node|traditions-as-mounts|incomplete-enumeration-in-facts|one-ruling-for-the-reading-class|hold-for-traditions-home|relation-per-option|re-pointing-checked|relation-per-holding|the-relation-is-projected-onto-the-option-as-one-of-three", but the fact's own opening prose paragraph, unchanged by this amendment, still reads: "`relation-per-option` is recommended because it is the author's words of 2026-09-04, quoted above, it is what the reader and the fifty-nine readings on the record already carry, and it is what makes chosen over derivable rather than stored." The 2026-09-07 option-adoption moved `recommends` off `relation-per-option` (now only `stands`) onto the new option, but the explanatory paragraph and the fact-level `against` ("The per-option relation binds a tradition's verdict to option names... a reading per node multiplies files...") were never rewritten to justify the new recommendation; they still argue for and against the option that now merely stands. Suggested edit: rewrite the fact's opening paragraph to state why `the-relation-is-projected-onto-the-option-as-one-of-three` is recommended (its own subsection's "AI support" already has the material), and either replace the fact-level `against` with that option's own "AI divergence" case or fold it in.
- The Frontier finding of 2026-09-05 (contradiction, 46 nodes), already recorded verbatim in this node's own '## Account' by this amendment, states: "`commons.systems/disposition-graph/readings` carries a variant of the same claim in its authority fact's `against`, at lines 47 and 155: 'every reading on the record recommends delegated for itself'" and proposes "Strike the census from all forty-six and from `readings`' `against`, replacing it with the rule rather than the count." This node's own `authority` fact still carries that exact sentence, unchanged by the diff, in both the summary line ("against: Delegated is live: the relations are the AI's readings, every reading on the record recommends delegated for itself, and the author may hold the rule of how tradition binds the record as loosely.") and the ##### authority prose ("The case against is delegated: the relations are the AI's readings, every reading on the record recommends delegated for itself, and the author may hold the rule as loosely as its instances."). The amendment answers the finding for other nodes by recording an option on `authority` but leaves the false census standing on the very node the finding named by name.
- The Frontier finding of 2026-09-05 (redundancy, stub-traditions vs. readings), also recorded in this node's own Account by this amendment, proposes: "derive the list of rationales carrying prose tradition lists rather than writing it down" and "Strike the numbers from both option texts." The amendment's own fix to the previous reading's F6 goes the opposite way: `#### incomplete-enumeration-in-facts` now reads "Measured on 2026-09-05, by the marker phrases the record actually uses and not by one of them: nine rationales carry a prose tradition list, five more carry one only in an account, and `stub-traditions` stands at the maieutic stage with a hand-maintained enumeration naming twelve, which its own `regenerate-enumeration` option already calls stale." This restates a fresh hand count instead of striking one, which is exactly the anti-pattern (two disagreeing hand censuses of the same population) the redundancy finding names. Suggested edit: either drop the specific counts (as F3's fix did for the readings-bearing-elsewhere count) and cite `readings` as the rule rather than a number, per the redundancy finding's own proposal, or explicitly note the tension and defer the exact figure to an instrument.
- The Frontier finding of 2026-09-05 (contradiction, standing-option-carries-a-subsection), recorded in this node's Account by this amendment, names `readings`' own `#### relation-per-option` subsection among the nine violating nodes and among the "six... urgent" cases a projection actually renders wrong, since the encoding rule is that the option named by `stands` omits its subsection (its sentence is the first sentences of `## Answer`). That subsection is still present, unchanged, opening "A reading stays a node under one node it bears on, with its own class, and its relation attaches to the options of the fact it bears on rather than to the answer..." -- a description of the change, not the answer's first sentences. This is left open because the finding itself says the fix is blocked on a ruling of `dialogue`'s `standing-option-carries-a-subsection` option, so it is not a fault of this session's judgment, but it remains an unaddressed item at this node.

On the facts and what they recommend: The diff moves the `answer` fact's `recommends` to the new option `the-relation-is-projected-onto-the-option-as-one-of-three` (moderate boldness) while `stands` stays `relation-per-option`, correctly triggering a `## Recommendation` fence with the amended answer text (the three-accumulations clause); it also adds two other viable options (`re-pointing-checked`, `relation-per-holding`) responding to earlier findings. But the fact's own explanatory prose and `against` were left describing the prior recommendation, not the new one (finding 1). The `authority` fact is untouched by this diff and still carries the false census the survey already flagged on this node (finding 2).
...
### Clean-context re-reading, 2026-09-07, of 428e3065
...
- ## Facts, ##### answer: the fact's own `against` is unchanged by the diff and still reads "The per-option relation binds a tradition's verdict to option names, which are the AI's handles and move under it as options are added, renamed, and composed, so the option the author confirms can show no tradition beside it while a rival shows one; and a reading per node multiplies files, whose queue effect is the cost the author will feel." That is the case against `relation-per-option`, the option that now only `stands`, not against `the-relation-is-projected-onto-the-option-as-one-of-three`, the option now `recommends`ed. Per the record's own vocabulary, `against` is "the AI's own case against the option it recommends," so this field is now describing the wrong option. The repair added a new opening paragraph justifying the new recommendation (closing half of the previous reading's finding 1) but left the `against` field untouched, which was the other half of that same suggested edit ("either replace the fact-level `against` with that option's own 'AI divergence' case or fold it in"). Suggested edit: replace or fold in the new option's own "AI divergence" sentence ("A projection is only as complete as the `bears` entries are...") as the fact-level `against`.
- ## Facts, ##### authority: the body prose still reads "The case against is delegated: the relations are the AI's readings, every reading on the record recommends delegated for itself, and the author may hold the rule as loosely as its instances." This is the exact census both the frontier survey of 2026-09-05 and the previous reading's finding 2 named as false, quoted from two locations on this node: the frontmatter summary line and this body prose. The diff (hunk under `facts:` / `recommends: ratified`) corrected only the frontmatter `against` field, to "each reading node recommends its own class for itself"; it left this identical body-prose sentence unedited, so the false census still stands in the rendered node. Suggested edit: apply the same correction ("each reading node recommends its own class for itself" or equivalent) to the ##### authority prose sentence.
- ## Account, ### Repaired after the re-reading, 2026-09-07: this section states "the authority fact's case against no longer claims that every reading on the record recommends delegated for itself," which is not accurate of the node as it now stands — the claim still appears verbatim in the ##### authority body prose (see the preceding finding). The account's self-report overstates what the repair actually did, crediting a fix that covered only the frontmatter copy of the sentence.

On the facts and what they recommend: The diff moves the `answer` fact's `recommends` to `the-relation-is-projected-onto-the-option-as-one-of-three` (moderate boldness) while `stands` stays `relation-per-option`, and prepends an opening paragraph justifying the new recommendation — correctly triggering the `## Recommendation` fence with the amended three-accumulations answer text — but leaves the fact's own `against` field arguing against the superseded option. The `authority` fact's `recommends` (ratified, moderate) and `stands` are untouched; only its frontmatter `against` text was corrected for the false census, while the identical sentence in the authority fact's body prose was left unedited.
