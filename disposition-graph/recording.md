...
review:
  ...
  strength: moderate
  date: 2026-09-07
  of: cf4d19756922122790f737224760f944d6dd4aa8
  commit: d35b0014367874f0376fea8e271f1737b798857c
  against: "The answer describes a recording no session can carry out. Its confirmation step enters the author's words 'in the ledger ... as the quotes node decides' and there is no ledger; the accumulation it now folds is `unconfirmed-accumulation`'s, whose own text says nothing it names is materialized and whose instrument `packages/disposition/accumulate.mjs` does not exist; and the readiness it gates waits on a survey that, by `review-cost`'s own measurement, no node in the record carries a pin from. The authority fact asks for ratified, so a confirmation would make doctrine of machinery that has never run, on the one node whose errors are, in its own words, 'an error in every ruling the record will ever hold' — and the correction would then have to be taken through the very machinery this answer defines. Against that: the answer states rules and not implementations, each thing it leans on is a node the author will rule on in its own turn, and the record's own test asks only that a draft say what it presumes unmaterialized, which is a disclosed clause and not a redesign."
  survey:
    date: 2026-09-05
