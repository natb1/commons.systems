...
defines:
  ...
  - rules
shims:
  - artifact: "`CLAUDE.md` on the implementation ref, the bootstrap operations document"
    for: the orientation page this node projects, carrying during bootstrap the operating rules, commands, session mechanics, and code-review recipe that no node yet projects
    liquidation: every rule this project runs under is a node or a declared shim, dispatch selects from this graph, the alignment skill is the only recording path, every materialized implementation has a completed review, nothing live reads the legacy record, and the projector regenerates the file from the purpose and projection nodes
    declared: 2026-09-02
---
## Answer
