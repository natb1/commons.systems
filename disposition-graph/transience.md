...
An un-aligned disposition, when the author has stated a disposition, or the AI holds evidence that needs the author's clarification, and the alignment dialogue on it has not concluded. It is a node with a question and no answer, hidden from the browser and listed by the alignment page. It carries the author's words, verbatim and dated, in a `## Disposition` section; the AI's account, its evidence, findings, and recommendation with the three facts, in a `## Proposal` section; and `stage`, the next movement of the dialogue owed on it: periagogic, maieutic, review, or ruling. The position is stored because it cannot be re-derived: what the author has said in the dialogue exists nowhere else, and it must survive the session that heard it. Every function of the unanswered node reduces to this. An owed reading or a surfaced term is not a node until it is written; it is a frontier item derived from the rationale that names it. An un-aligned disposition has no children, and at its recording it becomes an answer, an amendment quoted into the node it refines, or nothing. A sitting on an answered node carries the same fields while it is open, and the browser shows the node without them.
