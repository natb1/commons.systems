...
### Minted from growth's dialogue, 2026-09-07

The turn this node answers is the one the author's words of 2026-09-07 name, given
by the session on the draft of
`commons.systems/disposition-graph/authors-words-on-the-page`. It opened:

> `authors-words-on-the-page` turns on a measurement worth your eye, and it's recorded on the node so nobody has to retake it. Of 140 nodes, 68 carry your words — 222 dated entries.

The rest of the turn reported a repair of the parent node's bookkeeping and a
marking rule applied at two clauses. Nothing in it was a probe on the ground of a
question, a question about what the author intended, or a direction to the page,
which is what the author's reply says: the response "includes a lot of 'noise' and
it's hard for the author to know what to do with this".

The clean-context reading of `growth`'s draft of 2026-09-07 pointed the same way
on its own ground, before the author's sentence on `probe-or-node` was given. It
found that the four-forms rule "is a new question carried on this node's dialogue,
not a new answer to a question the record already asks", that no node in the index
of questions asks what form a turn addressed to the author takes, and that the two
nearest nodes carry the same rule from one side each,
`commons.systems/disposition-graph/author-questions` as
`the-sitting-stops-only-on-intent` and
`commons.systems/disposition-graph/alignment-page` as
`page-collects-only-the-confirmation`. The remedy it prescribed is this node: under
`growth`, carrying the author's words of 2026-09-06 and 2026-09-07, the four
forms, the exemption for what reconciliation outputs, and what the fourth form
contains, with `growth` keeping the loop and the two conducts and citing it. The
same reading's counter-argument on `growth` said what the merge would cost the
author: with one answer fact, they could not confirm the four forms without
ratifying nine other rules they were never asked about separately, and could not
refuse any one of them except by kicking the whole node back.

The author's words directed the split. On `probe-or-node`, 2026-09-07: "If
author's intent is a peristent disposition, then it may require a new node to be
reconciled into the alignment skill." The intent here is persistent, so the node
is minted and the reconciliation of the alignment skill to it runs under the grant
in the same message of 2026-09-07, "you have bootstrap authority to reconcile
alignment dialogue/review/survey/artifact", given for this sitting and for nothing
else.

What the node leaves to others: `growth` keeps the loop of three moves, the three
usages, the two conducts and their objects, and the movements of a sitting;
`author-questions` and `alignment-page` keep their options and cite this node
rather than each holding half of one rule; `commons.systems/disposition-graph/work-loop`
keeps the form of a reconciliation's own output.

The reading of this node's own recommendation is owed, which is what its stage
records.

### Clean-context review, 2026-09-07, of 69fa32a8
...
Recommended at this reading: `four-forms`.

Findings:

- ## Answer, third paragraph: "The rule is stated once, here, and the two nodes that carry it from one side each cite this node rather than restating half of it." That is not true of the record as it stands. `grep -rn "turn-form" disposition/disposition-graph/author-questions.md disposition/disposition-graph/alignment-page.md` returns nothing: neither node mentions this one. And `author-questions`'s option `the-sitting-stops-only-on-intent` does not carry one side of the rule, it restates the whole three-surface statement -- "The author's fuller words of the same day fix the three surfaces the dialogue has and no fourth: the periagogic movement establishes common grounding, the maieutic clarifies ambiguities in the author's intent, and the alignment artifact takes the confirmation of a disposition's facts" (`disposition/disposition-graph/author-questions.md` line 249-252). So the sentence that justifies minting this node -- that the rule is stated once -- is contradicted by the record it describes, and the author would be ruling on a claim of fact that is false. Suggested edit: put it in the mood it is in, and name the reconciliation as owed: "The rule is stated here and once. The two nodes that carry it from one side are amended to cite this node rather than restating half of it: `commons.systems/disposition-graph/author-questions`, whose `the-sitting-stops-only-on-intent` today restates the three surfaces in full, and `commons.systems/disposition-graph/alignment-page`, whose `page-collects-only-the-confirmation` carries the page's side." Then record the two amendments as options on those nodes, sourced to this one, so they are ruled and not discovered.
- Frontmatter, `question: What form does a turn addressed to the author take?`, against the answer's own scope: "what a reconciliation session outputs, a listing or a report included, is outside the rule altogether, which binds the alignment dialogue and nothing else". A reconciliation session's report is a turn addressed to the author, so the question as written asks for a rule this answer deliberately does not give, and an executor reading the question and the first sentence together -- "every turn addressed to the author takes one of four forms and no fifth" -- would apply it to reconciliation and then find the exemption three paragraphs later. Suggested edit: amend the question to "What form does a turn of the alignment dialogue addressed to the author take?", which is what the answer answers and what leaves `commons.systems/disposition-graph/work-loop` its half. The parent's citation pulls the other way and should be trimmed with it: `growth`'s recommended text says "What form a turn addressed to the author takes, and what the dialogue may spend the author's attention on, is the question of the node beneath this one, turn-form", attributing to this node a second question its question does not ask -- and by `commons.systems/disposition-graph/node`'s rule that a text answering two questions is two nodes, the second belongs either in this question or in a node of its own, not in the parent's citation alone.
- ## Facts, `##### answer`, the boldness: "Boldness low: the three surfaces are the author's words of 2026-09-06 and the four forms their words of 2026-09-07, close to as they stand. What is the AI's is at the enumeration's edges, and it is small." The AI's addition is not at the edges. The author wrote that dialogue prompts "are expected to take" the four forms; this answer writes "every turn addressed to the author takes one of four forms and no fifth". Converting an expectation into a bound is the whole operative force of the answer -- it is what makes a turn of a fifth kind a breach rather than a departure -- and it is the AI's, which is precisely why `expected-not-required` had to be recorded as an option. Boldness is a measure of how much rests on the AI's own knowledge against the author's words, so it is moderate. Suggested edit: set `boldness: moderate` and say in the fact's reason that the enumeration is the author's and the hardening of "expected" into "and no fifth" is the AI's.
- ## Answer, first paragraph: "A session with none of the four to give has nothing to stop for and proceeds." Read by an executor, that sentence licenses silent proceeding in the one case where proceeding is the wrong action: the session that cannot do what it was asked -- a landing that will not land, a worktree reaped under it, a graph that will not validate, an instruction that contradicts a ruling already recorded. None of the four forms fits an impediment: it is not a probe on the ground of a question, not a question about intent, not a direction to the page, and not an acknowledgement of a confirmation. As the rule stands the session must either invent a fifth form or say nothing and carry on, and the second is what the sentence tells it to do. The two exemptions already in the answer show the shape of the fix. Suggested edit: add a third thing standing outside the rule -- a turn that reports an impediment, which says in one line that the sitting cannot continue and what stopped it, and asks nothing; it is outside the rule because it is not the AI's account put before the author's but the sitting's inability to give them any of the four. A viable option carrying it is named in the `viability` field.
- ## Rationale, the tradition pass: "Both traditions support the rule and neither is diverged from". The claim is accurate for what the two reading nodes say -- `plato-periagoge` holds that the interview "does not implant an account but turns the author to the record and the readings, and the author articulates first", and `plato-maieutics` that Socrates "claims to bring forth what the interlocutor already carries and to contribute nothing of his own" -- but neither reading bears on this node: `## The readings that bear on it` is empty, and both are recorded under `growth` on `growth`'s options. `commons.systems/disposition-graph/evaluation` requires that "every tradition surfaced is recorded as a reading with the resolution it informed". Suggested edit: add a `bears` entry on each of the two reading nodes naming this node's answer fact and the option `four-forms` with relation `adopted`, so that the support this rationale claims is readable from the readings themselves and not only asserted here.

On the facts and what they recommend: The answer fact recommends `four-forms`, which is also what `stands`, so no `## Recommendation` fence is present and none should be -- the node was minted this sitting and its draft is its answer. The authority fact recommends `ratified` at low boldness with the `### authority` reading `class-recommendation` requires, and it is right: the rule fixes what the AI may say to the only party that checks it, which is the capture limb exactly, and the reading is honest in saying the other two limbs are not met. The answer fact's low boldness is wrong for the reason in the third finding; every other fact and its reason are in order, and the claims about the record are exact except the one the first finding names.

On the viability of the options: Three of the four options are viable and rightly placed: `expected-not-required` carries the softer reading the author's own word "expected" supports, and the two passed-over options each say why they lost. One viable option is missing. Its prose: `a-turn-that-reports-an-impediment` -- everything the recommended option says, with a third thing standing outside the rule beside the two already there. A turn that reports an impediment is outside it: where the sitting cannot continue -- the landing will not land, the record will not validate, the instruction contradicts a ruling already given -- the session says so in one line, names what stopped it, and asks nothing. It is viable because the rule as recommended leaves that turn no form and then tells the session that a turn of no form means it "has nothing to stop for and proceeds", which is the one case where proceeding silently is worse than any noise; and because the two exemptions the answer already carries are of the same kind, things that are not the AI's account put before the author's. It should be recorded with source `review`, ref 2026-09-07.

Strongest counter-argument (moderate): The rule is drawn from a single turn the author disliked, and the author's own words in that same message break it twice over: they say the four forms are what prompts "are expected to take", not what every turn must be, and they close by asking for a listing of queued recommendations, which is a turn of none of the four. The answer meets the second by exempting what the author expressly asks for and the first by simply choosing the harder reading, and the cost of choosing it is borne where it cannot be seen -- in the turns a future session does not make. A session that has found something the author would want to know, and that is neither a probe nor a direction nor an acknowledgement, is told to write it to the record and let the projections carry it, on the ground that "a finding that cannot survive being written down was not worth the author's turn"; but survival in the record is not the same as reaching the author, and the record is already large enough that what is written to it is read when the author happens on it and not when it matters. What the author asked for was less noise, and what is recorded is a closed list of four forms with two exemptions and, as the fourth finding shows, no form at all for a session that cannot proceed.

The session's reply: Accepted on all five, each verified at its locus on the main thread: neither author-questions nor alignment-page names this node, and the-sitting-stops-only-on-intent restates the three surfaces whole, so the sentence that the two nodes cite this node is false as the record stands; the question asks about every turn addressed to the author while the answer binds the alignment dialogue alone; the hardening of the author's expected into no fifth is the AI's and is the whole operative force, so boldness is moderate; a session that cannot proceed is told to proceed silently, since none of the four forms fits an impediment; and the two Platonic readings the rationale cites bear on growth and not on this node. The amendments owed: the sentence put in the mood it is in and the two amendments named as owed, with the option cites-turn-form-for-the-three-surfaces recorded on author-questions, source review, and alignment-page amended in the same sitting to cite this node where it states the page's side; the question amended to a turn of the alignment dialogue addressed to the author; the growth citation left standing, since what the dialogue may spend the author's attention on is the same question put from the other side and not a second one, which the rationale says; boldness moderate with the reason saying the enumeration is the author's and the bound is the AI's; the option a-turn-that-reports-an-impediment recorded, source review, and the recommendation moved to it, a turn of one line saying the sitting cannot continue and what stopped it, standing outside the rule beside the two exemptions because it is not the AI's account put before the author's; and bears entries on plato-periagoge and plato-maieutics naming this node's answer fact. On the counter-argument: the author's word expected is carried by the option expected-not-required, which stays viable, and the counter-argument goes on the row at the strength the reading gave it. The amended answer owes its re-reading.

### Amended after the reading, 2026-09-07

All five findings are applied. The recommendation moves from `four-forms` to
`a-turn-that-reports-an-impediment`, so the amended text stands in the
`## Recommendation` fence and `## Answer` keeps `four-forms` as first recorded,
which is what `stands` names. What each finding did. The sentence that the two
nodes cite this node is put in the mood it is in and names the two amendments as
owed, one recorded on `commons.systems/disposition-graph/author-questions` as the
option `cites-turn-form-for-the-three-surfaces`, source review, viable and not
adopted, since that node is at the ruling stage and its recommendation does not
move; the other made in this sitting on
`commons.systems/disposition-graph/alignment-page`, whose page-scope clause cites
this node where it states the page's side. The question becomes "What form does a
turn of the alignment dialogue addressed to the author take?", and the answer's
first sentence is qualified with it, so that the scope the answer takes is stated
where an executor meets it first; `growth`'s citation is left as it stands,
because what the dialogue may spend the author's attention on is this question
from its other side and not a second one, which the rationale now says. Boldness
moves to moderate, the reason saying that the enumeration is the author's and that
the hardening of "expected" into "one of four forms and no fifth" is the AI's and
is the answer's whole operative force. The impediment turn is recorded as an
option and recommended, a turn of one line saying that the sitting cannot continue
and what stopped it, standing outside the rule beside the two exemptions because
it is not the AI's account put before the author's but the sitting's inability to
give any of the four. `bears` entries naming this node's answer fact and the
recommended option, relation adopted, are recorded on
`commons.systems/disposition-graph/plato-periagoge` and
`commons.systems/disposition-graph/plato-maieutics`, which are read under `growth`
and until now bore on nothing here.

Two consequences of the move, recorded so they are not read as substance. The
three subsections written against the recommendation when it was `four-forms` —
`expected-not-required`, `three-forms-no-acknowledgement` and `rule-on-growth` —
open by naming `four-forms` rather than "the recommended option", which is what
they were always about; and `four-forms` gains a subsection of its own, which the
option that stands may carry, saying that it is the rule without the impediment
turn and that the reading's three other corrections are carried in the recommended
text and are not what separates the two options. The fact's case against is
rewritten to what the reading's counter-argument says of the new recommendation:
the impediment turn answers the sitting that cannot continue and does not answer
the session that has found something the author would want to know.
