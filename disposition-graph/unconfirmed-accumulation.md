...
**What the node carries.** Its question, its `form`, its `under`, its `depends`, its `defines`, its shims and its instrument, which are its identity and every edge a projection walks. Every fact, with the option it recommends, the boldness of that recommendation, the strongest case against it, and the fact's own reason, which is the judgment that weighs the support and the divergence across the options. Every option of every fact, with its name, its source and ref, its status and the reason it was passed over, its ruling where the author has given one, the references to the author's words that support it and the references to those it diverges from, the AI's accumulated support and its accumulated divergence kept whatever the fact recommends, and, on the answer fact, its content, whole or as a named change to another option's. The `stage`. And the `review`, the draft reading's keys and the survey's block. Nothing here is stored that can be derived: the confirmed label is not, the class is not, the tradition's support and divergence on an option is not, being the derived inverse of what the readings bear on, and the node's answer is not, being the resolved content of the option last confirmed.

**What is folded, and what is absorbed.** A section is absorbed where its content is material the dialogue needs in a place the dialogue does not read, and it is moved there rather than struck: the node-level `## Rationale`, whose argument goes to the recommended option's support, to the rivals' divergence and to the fact's own reason, and whose traditions go to the readings that bear on the options they inform; `## Disposition`, whose entries go to the ledger and reach the options as references, retained on an option only where they support it or it diverges from them; and an account section whose findings are now an option's divergence. A section is folded where its content is the dialogue's history and nothing reads it twice: every `## Account` section but the last, and a superseded `review` block. Nothing else. The fold has a list and does not have a size.
