...
the choice is no confirmation and is not recorded as one: the session classifies
it as the recording node classifies every response given in prose, and it
classifies as the denial the author's words make it, carrying their words as the
feedback, and saying which reading it took in the turn it takes it. The label is
...
the review stage, so the correction costs one further confirmation and not a
redrawing.
...
`disposition` ref as read on 2026-09-06; the fence runs 469-539 and each quoted
