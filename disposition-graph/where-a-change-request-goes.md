  ...
  - name: answer
    options:
      ...
      - name: edits-ride-on-the-option
        ...
        ref: "2026-09-05"
        status: passed
        reason: "the author's words of 2026-09-04 strike it by name, and it makes a confirmation carry a text the author confirmed in the same breath as changing it"
...
kick-back's feedback control, on the fact it bears on, and that control is to ask for
the change and not only for the ground of the refusal, which is
`when-the-kickback-feedback-shows`' to rule and is recorded there.
...
marking and its feedback control, the clause beginning "The last row on every
fact is the kick-back", devolved to `when-the-kickback-feedback-shows`, and the
simile in the clause beginning "The right-hand column is the disposition and
nothing else", which survives the change. Its requirement on the kick-back control is carried in the amendment
...
and on `recording`, for their own rulings; the `depends` entry this node once
carried on the second was dropped on 2026-09-07, since this answer acts whichever
way `recording` rules.
...
carried them; the passed reason on `edits-ride-on-the-option` was rewritten in its
subsection to the author's words and, until the reading of 2026-09-07 found it,
not in the frontmatter field the page renders; and the reach into
...
### Clean-context review, 2026-09-07, of 3669f862
...
- Facts (validation 3), and an account that reports a repair it did not make; on the domination of the one whole alternative. The frontmatter reason on `edits-ride-on-the-option` still reads "it makes a confirmation carry a text the author did not confirm, and the pin can name only one of the two", while the `## Answer` now says of that very argument "There is no dilemma and no third pin, and the claim is withdrawn rather than restated", and the account's "### The second kickback, and the method changed, 2026-09-06" asserts "the passed reason on `edits-ride-on-the-option` no longer rests on the contradicted pin ground". The `#### edits-ride-on-the-option` subsection was rewritten and rests on the author's words; the frontmatter field was not, and the frontmatter field is what the author reads: `packages/disposition/project.mjs:1506` renders `passed over: ${o.reason}` on the option's row. So the author is shown the withdrawn argument as the reason the record's single whole alternative is out, which is the one place a wrong reason costs a ruling they might have given. Edit: set the frontmatter reason to the subsection's ground — "the author's words of 2026-09-04 strike it by name, and it makes a confirmation carry a text the author confirmed in the same breath as changing it" — and correct the account sentence to say what was actually changed. This field is not covered by the answer fact's pin, which `dialogue`'s `every-part-in-the-record` fixes as the fact's name, its recommended option, its boldness, the reason in `## Facts`, that option's sentence and the recommended text, so the amendment rides on this forward.
- Facts (validation 3), line anchors, the third reading in a row on this node. The account's "### The maieutic, 2026-09-06" still says "the kick-back's marking and its feedback control at line 499, devolved to `when-the-kickback-feedback-shows`, and the simile at line 505". Measured today in `disposition/disposition-graph/alignment-page.md` (`grep -n '^```' `: the fence runs 498-575, not the 498-568 the account's "### The kickback answered" section records): line 499 is the fence's own opening frontmatter delimiter `---` and line 505 is the shim's `for:` line. The correct anchors are 528 ("The last row on every fact is the kick-back, and it is not an option ...") and 534 ("The right-hand column is the disposition and nothing else ...", which carries the declined simile). The same section of the account that claims "the stale line anchors go with the amendment that carried them" leaves these two behind. Edit: locate both clauses by their words, as "### What a ruling here would reach in the parent" now does, or re-anchor to 528 and 534 and pin the graph commit beside the date; line numbers into a node that is itself on the frontier go stale between the sitting and the reading, which has now happened three times here.
- Cross-reference, an accepted finding neither applied nor answered. The frontmatter still carries `depends: commons.systems/disposition-graph/recording#edits-only-through-the-kick-back`. The reading of 2026-09-06 of a8bdef71 asked for it to be dropped ("drop this node's `depends` entry, since the conditional sentence acts whichever way `recording` rules"), the session's reply opens "Accepted", and neither the reply nor "### The second kickback, and the method changed, 2026-09-06" — which counts "Eight findings, three of them blocking, all validated at their loci" — says anything about it. The entry is now a live cost: `dialogue` gives `depends` the job of letting the page "refuse to put a question before the one it rests on", `recording` stands at the maieutic stage, and this node's own answer is expressly independent of how it rules ("Where the author rules that option the words are classified as the denial they make; until they do, this answer says only that the page offers no route for them"). So the ruling on this node is held behind a node it does not wait on. Edit: drop the `depends` entry, or say in the account why it is kept after being accepted for removal. A finding for another node, not an edit here: `recording`'s `#### edits-only-through-the-kick-back` still reads "the sitting the author queued for those words, `commons.systems/disposition-graph/where-a-change-request-goes`, has not yet been held", which was true on 2026-09-04 and is not now; that prose is owed the amendment that the sitting was held on 2026-09-06 and what it concluded.
- Doctrine and executor (validation 2), on the one sentence that reaches a sibling's question. The `## Answer`'s first paragraph states flatly that the kick-back's "control asks for the change and not only for the ground of the refusal", while `commons.systems/disposition-graph/when-the-kickback-feedback-shows`, which owns that control and stands at the periagogic stage, carries the same requirement only as the unruled option `the-control-asks-for-the-change-by-name`, whose own prose says "A ruling here is what would make it". The account admits the reach ("One clause this answer's requirement touches and a ruling here must not move ... recorded there as an option and not amended here") but the standing text does not, and an executor reading the answer alone would rewrite `KICKBACK_PLACEHOLDER` at `packages/disposition/project.mjs:681`, today "What these options miss, and what the next ones are drawn from", on this node's authority. It does not block, because both sides of the relation are recorded and `kick-back-ask-unchanged` lets the author rule the reach away without rejecting the routing. Edit, if taken: "... and that control is to ask for the change and not only for the ground of the refusal, which is `when-the-kickback-feedback-shows`' to rule and is recorded there". Note that this edit changes the `## Answer` and so the option's own sentence, which the answer fact's pin covers, so it cannot ride on this forward: take it and re-read, or leave the sentence and let the author rule `kick-back-ask-unchanged`.
- Readings (validation 4), a tradition already under the parent that bears on the clause this answer fixes and is not cited. `commons.systems/disposition-graph/chenery-reasoned-decision` asks "What does the duty of reasoned decision say about an optional reason beside a ruling", is `relation: diverged` on `alignment-page#every-fact-every-option` and on `dialogue#every-part-in-the-record`, and says in terms that "Where the page diverges is on the reason for the choice: the text control in the option's drill-down is optional, so a ruling may be recorded with the choice and no ground at all". This answer's first sentence fixes that control's label — "it asks for the reason, says it is optional, and names no edit" — and carries no reading at all, so a ruling here would ratify the optionality on the node that specifies the label with the divergence recorded only on an unruled option of the parent, which `evaluation` treats as an unrecorded conflict deferred to neither side. Edit, a finding about another node: record on `chenery-reasoned-decision` a third `bears` entry, `{node: commons.systems/disposition-graph/where-a-change-request-goes, fact: answer, option: change-requests-go-to-the-kick-back, relation: diverged}`, so the divergence travels with the clause; or drop "says it is optional" here as the parent's clause ("on an option the words are optional because the ruling's content is the option") rather than this answer's.

On the facts and what they recommend: The answer fact recommends `change-requests-go-to-the-kick-back`, which is also `stands`, so there is correctly no `## Recommendation` fence and the draft is the node as it stands; nine options, five passed with reasons, and moderate boldness is right, the routing and the label being the author's words nearly verbatim while the reach past the page is the AI's, which is what the fact's `against` argues with. The authority fact recommends `ratified` at low boldness with the written `### authority` reading `class-recommendation` requires, and capture-shaped is the correct limb, the reading disclaiming expensive and irreversible rather than claiming all three; no `existence` or `persistence` fact is owed, since no prune is proposed and the node's shape does not change, and `node packages/disposition/validate.mjs disposition` returns `ok: 140 nodes`. On validation 15 nothing is owed: the two answers this draft implies elsewhere are already recorded as options on the nodes whose questions they answer, `two-responses-where-the-page-offers-them` on `unanswered` and `the-control-asks-for-the-change-by-name` on `when-the-kickback-feedback-shows`, both verified present. The standing review pin (kickback, of a8bdef71) is the pin of the reading this draft answers and is stale by design; findings 1, 2, 3 and 5 touch a passed option's frontmatter reason, the account, `depends` and another node's `bears`, none of which the answer fact's pin covers, so they ride with this forward, while finding 4 would move it and is marked as optional for that reason.

On the viability of the options: Every option on both facts is viable on its facts and the answer fact's set covers the space: the incumbent `edits-ride-on-the-option` and its strongest form `edit-applied-and-held-until-the-re-reading`, the two half-measures `label-only-change` and `a-dedicated-change-request-control`, the two placements `one-reason-box-per-fact` and `reason-goes-to-the-interview-too`, the typing variant `change-request-typed-to-review`, and `kick-back-ask-unchanged`, which lets the author take the routing without ruling on a control that belongs to `when-the-kickback-feedback-shows`. The three vocabulary options on the authority fact are complete. No viable option is missing; what is wrong is a reason and not a list, as finding 1 says — the domination of the one whole alternative is still argued in the frontmatter on the pin ground this answer withdraws, and that is the field the page renders.

Strongest counter-argument (moderate): With the pin ground struck the answer's whole reach rests on one parenthesis in a note about a text input, and it reads that parenthesis as closing, on the author's only ruling surface, one of the three responses `unanswered` opens everywhere. The parenthesis is satisfied by a smaller answer: `label-only-change` plus the routing gives the author exactly the control they asked for, and the difference is paid on every wording correction they will ever want, since a kick-back on one fact moves the whole node, returns it to the maieutic movement, buys a redraft and a clean-context re-reading before any ruling is recorded, and so routes the cheapest class of feedback down the most expensive path the record has. The answer's own deference makes it worse rather than better: it leaves the classification to `recording`, whose recommended `per-fact-after-two-readings` says that words amending an option "are edits, and are applied to it" and that "A confirmation with edits is a ruling like any other and is never held", and pins `of` to the recommendation as the edits leave it — so if `recording` stands, the record will apply an edit written in a control whose label, fixed here, tells the author it takes no edits, which is a trap of this answer's own making. And the cost is not symmetrical for the author: the option the answer removes is the one the author uses when they agree with the draft, and what replaces it discards the ruling they were ready to give.

The session's reply: Accepted on four findings and the fifth taken. The frontmatter reason on `edits-ride-on-the-option` still read the withdrawn pin ground while the subsection and the answer had struck it, and the account claimed the repair: the reason now states the subsection's ground, the author's words of 2026-09-04 striking it by name, and the account sentence is corrected to say the subsection was rewritten and the field was not until this reading. The two anchors at 499 and 505 were measured stale, the fence now running 498-575, and the clauses are located by their words. The `depends` entry on `recording#edits-only-through-the-kick-back` was accepted for removal on 2026-09-06 and not removed; it is dropped now, since the answer acts whichever way `recording` rules, and the stale sentence on `recording`'s option, that this sitting has not yet been held, is amended there to say it was held on 2026-09-06 and what it concluded, which moves no recommendation on that node. The third finding is taken although it moves the pin: the answer's sentence on the kick-back's control now names `when-the-kickback-feedback-shows` as the node that rules it, so an executor reading this answer alone rewrites nothing on this node's authority, and the amendment is read once more. On the fourth, the reading misstates one relation, `chenery-reasoned-decision` bears on `dialogue#every-part-in-the-record` as adopted and not diverged; the edit is right regardless, and the reading node gains a `bears` entry on this node's recommended option as diverged, since the divergence travels with the clause that fixes the control's optional reason. Against the counter-argument the answer stands on the author's words: the label-only route it prefers keeps an edit channel the author struck by name on 2026-09-04, and the price it names, a kick-back on a wording correction, is the author's own choice recorded on the node, stated in the answer as a consequence and not hidden; the trap it describes, an edit applied from a control whose label says it takes none, is the conflict between `recording`'s recommended text and the author's words that `recording` already carries as `edits-only-through-the-kick-back`, and it is `recording`'s to settle.

### Clean-context re-reading, 2026-09-07, of d4f8ef94
...
- Account, introduced by the amendment; the paragraph the amendment edited still names the field the amendment removed. "### The second kickback, and the method changed, 2026-09-06" reads, at disposition/disposition-graph/where-a-change-request-goes.md:402-404, "that consequence is on `unanswered` and on `recording`, for their own rulings, and this node's `depends` names the second." The amendment dropped the `depends` block from the frontmatter (the diff's `-depends:` / `-  - commons.systems/disposition-graph/recording#edits-only-through-the-kick-back`), so the sentence now states of the node in front of the author a dependency the node does not carry, and `grep -n depends` on the file returns it only in account prose. The session touched this very paragraph in the same pass — the "The rest:" list two sentences below was rewritten to say the frontmatter reason "was rewritten in its subsection ... and, until the reading of 2026-09-07 found it, not in the frontmatter field the page renders" — so one present-tense claim in the paragraph was corrected and its neighbour was not. This is the fourth reading in a row on this node to find an account sentence that its own text falsifies, and it is the class of defect the node's own "### The second kickback" section names ("the third node of this sitting whose account asserted a repair its text did not carry"). It does not block: the account is not covered by the answer fact's pin, which `dialogue`'s `every-part-in-the-record` fixes as the fact's name, its recommended option, its boldness, the reason in `## Facts`, that option's sentence and the recommended text, so the correction rides on this forward as the last reading's own non-pin findings did. Edit: "... and on `recording`, for their own rulings, and this node's `depends` named the second until the reading of 2026-09-07, when it was dropped as accepted on 2026-09-06 and not then applied." Secondary, in the same paragraph and fixed by the same edit: "the stale line anchors go with the amendment that carried them" is still the account's description of a repair the amendment did not in fact complete — the two anchors at 499 and 505 stayed behind and were fixed on 2026-09-07 by locating their clauses by their words, not by the withdrawn amendment. The anchor finding itself is answered and closed; only the account's description of how it was answered is stale.

On the facts and what they recommend: The diff moves nothing about what either fact recommends: `answer` still recommends `change-requests-go-to-the-kick-back` at moderate boldness with that option still `stands`, `authority` still recommends `ratified` at low, both `against` texts are unchanged, and there is correctly no `## Recommendation` fence. Inside the facts it changes exactly one field, the frontmatter `reason` on the passed option `edits-ride-on-the-option`, now "the author's words of 2026-09-04 strike it by name, and it makes a confirmation carry a text the author confirmed in the same breath as changing it", which is the last reading's own suggested wording and matches the `#### edits-ride-on-the-option` subsection; and, through the `## Answer`'s first paragraph, it changes the `stands` option's own sentence, which the answer fact's pin covers, so the recorded forward pin of 3669f862 is stale by design and this reading is what a new pin rests on. `node packages/disposition/validate.mjs disposition` returns ok: 140 nodes with the `depends` block gone.

On the viability of the options: Every option on both facts remains viable and none joined or left either list: nine on the answer fact, five passed with reasons, and the three vocabulary options on `authority`. The only viability-bearing change is the rewritten domination reason on the record's single whole alternative, which now rests on the author's words of 2026-09-04 rather than on the pin ground the `## Answer` withdraws, so the incumbent the author might still rule for is shown as passed over on a ground the record supports rather than one it contradicts.

Strongest counter-argument (moderate): Every instance the last reading named is repaired, but the class of defect it was raised for is not: the amendment falsified a sentence of the account in the very paragraph it was editing, so for the fourth reading running this node reaches the author with an account that describes a state its own frontmatter does not carry, and the author cannot check the record from the page if the page's account of the record is what needs checking. On the first finding the repair is also the reading's own words rather than the reading's own test: "it makes a confirmation carry a text the author confirmed in the same breath as changing it" is the withdrawn pin objection with the word "pin" removed, and `recording`'s resolution — pinning `of` to the recommendation as the edits leave it, so the text confirmed is the author's own — answers that sentence as squarely as it answered the one struck from the `## Answer`. What holds the finding closed is the reason's first limb, the author's words of 2026-09-04 striking the option by name, which dominates the incumbent without the second limb; and what holds the verdict is that neither residue touches what either fact recommends, so both are correctable before the author rules without buying a redraft and a fourth reading.

The session's reply: Accepted. The sentence in the account that named the `depends` entry the amendment dropped is corrected to say the entry was dropped on 2026-09-07 and why. Against the counter-argument: the class of defect, an account describing a state the frontmatter does not carry, is the one this sitting has named on `recording` as a failure mode, and the cure it takes there is what was taken here, the live text edited and the account checked by search over it; this reading found one sentence that search did not reach because it named the field in prose rather than by its key, and that is the second reading of this answer, so the sentence is repaired and no third reading is run.

### Frontier survey, 2026-09-07, of d4f8ef94
