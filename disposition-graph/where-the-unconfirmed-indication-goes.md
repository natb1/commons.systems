...
review:
  ...
  strength: none
  date: 2026-09-07
  of: a3ee2ba1be4ade16a0fa11d323bcb6cb56ba7377
...
### Clean-context review, 2026-09-07, of 13a8ddc7

Read in clean context by a subagent given this draft, its ancestry, its siblings, the nodes it names, and the index of every question the record asks, and nothing of the sitting. Verdict: forward to the author's ruling.

Recommended at this reading: `on-the-answer-fact`.

Findings:

- ## Recommendation, `## Answer`, first paragraph, against the sibling drafted in this same wave. The line is "written wherever that fact carries no ruling, and nowhere else on the page", and this node's own account measures what that means: "No node in the record carries a ruling on any fact, so the line is written on every one of the 134 today." In the same movement, on the same page, `commons.systems/disposition-graph/vocabulary-option-summary` demotes a sentence off the row for exactly that property: "a sentence carried on 134 of the 141 node files of both graphs is read once and skipped thereafter, so the gloss earns its place on the first node and costs on every node after it". The two drafts apply opposite rules to the same page on the same day, one taking a sentence off 134 nodes because it repeats and the other adding an identical sentence to 134 nodes. The option's own `against` half-concedes it -- "until the first ratification the line never varies, so it faces the same objection as an unvarying heading" -- but the answer never meets the sibling's rule head-on. Suggested edit: say in the Answer why repetition disqualifies the authority gloss from the row and does not disqualify this line from the fact, which the answer can do (the gloss explains a choice the reader already made once, and this line warns about an act each node repeats) -- or take the narrowing named in the viability field below.
- ## Facts, answer fact, the `against` on `on-the-answer-fact`, and ## Account, last paragraph. Both say `how-a-fact-is-headed` is at the periagogic movement: "the section it sits in is the one whose heading is `how-a-fact-is-headed`'s question, at the periagogic movement, where the author's words ask for that section to carry less and not more", and "It lands in the same function as `how-a-fact-is-headed`, which is at the periagogic movement and owns the heading of that section but not what sits beneath it". That node is at the review stage in this same wave, carrying a recommendation, `name-linked-to-the-definer`, that replaces the heading of exactly this section with the fact's name linked to its definer. The claim about the record is therefore stale, and so is the inference drawn from it: the sibling's answer does not ask the section to "carry less", it shortens the heading and moves the question behind a link, which is neutral about a line beneath the heading. Suggested edit: say that `how-a-fact-is-headed` is at the review stage recommending `name-linked-to-the-definer`, and that this line sits beneath a heading that answer shortens, so the two are complementary rather than in tension.
- A finding about the parent rather than about this node's text, and it is the parent clause most directly about this node's subject. `commons.systems/disposition-graph/alignment-page` still says the row carries this, unmarked and undevolved: "A choice that keeps the text already in the record is named for the authority that text has and never for more, and that name is its status. Where the answer is ratified, the choice is the answer as ratified and a confirmation keeps it. Where no ruling stands on the answer fact it is a draft no one has confirmed ... since that ruling is about who decides and not about this text, so the row says that confirming ratifies the AI's draft." Under this answer and under `what-an-option-row-carries` the row says none of it. The parent marks three other clauses as standing only until a child rules and does not mark this paragraph, so a reader of the parent meets an unqualified rule that two of its children have replaced. Suggested edit on the parent: mark the paragraph, in the form the other three already take, naming both children.
- ## Account, the readings this node has consumed. "The fifth kickback answered by moving the answer, 2026-09-07" -- and the re-reading of the moved recommendation makes a sixth reading owed on one node. `recording`'s recommended text sets the cost rule in terms: "A draft gets two readings, the reading and the re-reading of the amendment that answers it, and no more of one answer; the re-reading's object, and the cap, are the review-cost node's." The Account's escape is written and is the right one -- "the redraw is a new answer and not a repair" -- but it is asserted rather than tested, and `commons.systems/disposition-graph/review-cost` is in this same round, now recommending `one-answer-a-node-and-one-read`, so what counts as a new answer for the purposes of that cap is being decided beside this node. Suggested edit: say which rule the sixth reading runs under and cite `review-cost` for the cap, so a session applying this reading does not have to infer the exemption.
- ## Recommendation, `## Answer`, validation 3 and the measurement. I re-took every figure and every citation and they are exact, which is worth recording since this reading is the sixth. At graph commit `5da05bc4`: 141 node files, 33 carrying both an answer and a recommended text, 88 an answer alone, none a fence alone, 20 neither, so 121 columns rendered; 134 carrying an answer fact, the 7 that do not all among the 20 that render no column, and 13 carrying an answer fact with no text standing. No node carries a ruling on any fact -- the only `ruling:` key in the graph is inside an illustrative fenced block in `dialogue`'s prose at `disposition/disposition-graph/dialogue.md:1361`, which is an example of the superseded `adopts`/`choices` encoding and not a ruling. The graph citations are exact too: `dialogue.md:784` is `edit-led-against-a-named-ground`'s prose, `:788` is the `#### every-part-in-the-record` heading, and `:972` carries the clause quoted verbatim. `standingState` does key on `ratified` alone (`packages/disposition/project.mjs:701-706`). The one thing to note is that unlike the other three drafts in this wave, this one carries no line citations into `packages/disposition/project.mjs`, so none of it staled when `cb0e02c6` landed on 2026-09-07; where it names the projector it names functions, `renderPane`, `PANE_LBL`, `edit-lbl`, `standingState`, `renderFact`, which is the form that does not go stale and which the other three should take.
- ## Recommendation, `## Answer`, the paragraph beginning "One line, once per node, and only where a confirmation can be staged." The sentence promises less than the paragraph delivers, and the ambiguity is load-bearing. Its own gloss takes "can be staged" to mean that the node carries an answer fact with options -- "A node that carries no facts carries no answer fact, offers no option to confirm, and takes no line" -- but the natural reading of the phrase is that the control is live, and on that reading it is false: the paragraph writes the line wherever the answer fact carries no ruling -- "Where a node's answer fact carries options but no text stands, the line is still written, since what it says is that no ruling stands on the answer and not that a column is rendered" -- and the parent holds that a confirmation cannot be staged before the ruling stage at all: "A confirmation recorded on a node that has not reached that stage is invalid, as the author's words of 2026-09-04 direct, so at every earlier stage the facts, their options and the recommendation are all rendered and every input among them is disabled." So on every node at the periagogic, maieutic or review stage the line is written where, by the parent's own clause, no confirmation can be staged. Either the opening sentence overclaims or the rule is wider than the reason given for it. Suggested edit: strike "and only where a confirmation can be staged" from the opening sentence and say instead "and only where a node's answer fact carries no ruling"; or take the narrowing in the viability field, which makes the opening sentence true.

On the facts and what they recommend: Two facts. The answer fact recommends `on-the-answer-fact` at moderate boldness among five options, two passed over with reasons, while `stands` still names `the-line-that-names-what-the-pane-shows`, so the `## Recommendation` fence is correctly present and carries the frontmatter without the dialogue's keys, `## Answer` and `## Rationale`. Moderate boldness is right and the fact's prose locates it precisely -- the author fixed the cardinality and left the placement, and the record rules out two of four candidates and neither of the remaining two. The authority fact recommends `ratified` at low boldness with the three reserved terms and an `### authority` subsection that applies `class-recommendation`'s limbs by name and narrows the capture claim onto the new object, which is the repair the earlier reading asked for. The review state is the one thing a session must handle before this goes to the author: the node carries `verdict: kickback` pinned to `8b1d076b07dab5fb9fdd5e25f78f9acb65467ce7`, which the recommendation has moved past, so the pin is stale and this reading is what replaces it.

On the viability of the options: Every listed option is viable and the two passed over carry reasons that hold: `warning-in-the-eyebrow` is struck by the parent's exhaustive naming of that line's contents, and `no-per-node-warning` is the state the author objected to on `commons.systems/public/agency`. `warning-on-the-stage-chip` is rightly kept viable and its treatment is fair to itself. One viable option is missing, and it answers both this draft's own repetition objection and the round-level one in the first finding: `line-only-where-the-act-is-live` -- everything the recommended option says, with the line written on the answer fact only at the ruling stage, where the controls are live and a confirmation can actually be staged. The parent supplies the ground in terms: "at every earlier stage the facts, their options and the recommendation are all rendered and every input among them is disabled: the author sees exactly what will be asked and cannot yet answer it." The answer's whole case for the placement is that the warning must be in the path of the act -- "where they will be standing when they can do the thing it warns about" -- and at the three earlier stages they cannot do it, so on those nodes the line is a sentence about an act the page will not accept. Narrowing it there cuts the repetition sharply, makes the answer's own opening sentence ("only where a confirmation can be staged") true rather than aspirational, and costs one condition in the same branch of `renderFact` the answer already asks for. It is not dominated by anything on the list, and the author should get to rule on it.
