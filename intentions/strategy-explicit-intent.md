...
kind: strategy
statement: Govern by an explicit, self-describing intention graph
owner: human
status: refining
parent: null
rationale: >-
  The graph is the materialization of the one non-delegable virtue:
  virtue-philosophical-mobility is the discernment that monitors every
  delegation and judges recovery, and this graph is where that discernment is
  externalized — the delegation records, the tension attributes, the standing
  ...
  none — it becomes the corrupted sensor the virtue warns about, reporting
  not-captured from old data — so maintenance must stay cheap enough that the
  graph tracks reality; as a ratchet tooth the graph itself passes
  strategy-reversible-institution's test, since markdown archives by inaction.
  And the graph is drafted largely through the delegatee under audit
  (delegation-anthropic-claude): virtue and strategy substance must remain
  human-authored, with agent assistance as drafting, never derivation — imported
  language reshapes how problems are perceived, which is also why
  strategy-external-calibration's challenges extend to a node's framing, not
  just its priority.
...
recovers: []
clarifications:
  - question: What is the graph's function beyond keeping intent explicit?
    answer: "A dual map and a model. Map of author intention: the goal layers —
      virtues (dispositions held) generate strategies against present
      conditions; strategies decompose to tactics (conditional intentions). Map
      of author knowledge: the examined-understanding apparatus on the same
      nodes — status (delegated vs codified), reading/gap, tradition records,
      attributes.grounding marks. Three functions follow: maintaining the
      author's own alignment with the good (strategy-philosophical-grounding,
      strategy-complete-grounding, the reading curriculum), avoiding capture by
      delegatees (virtue-philosophical-mobility, the delegation ledger,
      strategy-external-calibration), and aligning delegatees with the good —
      including very-long-horizon coding delegation
      (strategy-graph-drives-dispatch) and maintenance of the graph itself.
      Recorded 2026-07-08 interview."
  - question: In what sense is the graph the author's model of the good in a
      platonic sense?
    answer: "The sense the 2026-07-07 chunk-1 amendment on tradition-plato fixed:
      there is one Good — external, never fully realizable — and the graph is
      the author's best current realization of it. The virtue forest is the
      Good's imperfect constituent model; strategies are imperfect
      implementations of virtues under present conditions; tactics of
      strategies. Forms-as-action-blueprints stay declined, and the root layer
      stays deliberately unranked because realization is imperfect — the
      inside-the-graph apex question stays held open on
  ...
  - question: Is the knowledge map propositional — are the roots the author's
    ...
    answer: "Held provisionally as no: kind-virtue reads roots as dispositions held
      (hexis), with knowledge tracked by the surrounding apparatus, per
      tradition-plato's virtue-as-knowledge (Protagoras) divergence. That
      divergence is itself an unverified delegated articulation — the author has
      not read the Protagoras — so the position is held on trust pending
      personal reading; the deferral is tracked by
      tactic-reading-chunk-23-protagoras-virtue-knowledge, and this
      clarification re-opens if the reading overturns the divergence. Recorded
      2026-07-08 interview."
  - question: Does the periagoge education doctrine extend to AI delegatees — is
      Claude turned toward the good by injecting the graph into the harness?
    answer: "Yes, with three qualifications. The delegatee holds durable state at
      two levels — parameters (habituated by Anthropic's own virtues,
      strategies, and delegations) and harness (the graph, rules, memory — the
      part the author writes) — and graph injection turns a power already
      present (Republic VII 518b-d as amended), with verification external
      because self-report is untrustworthy while misdirected (518e-519b).
      Qualifications recorded rather than hidden: (1) no self-grown path — the
      turning is always educator-initiated, so it is cadence-dependent and an
      injection lapse drifts the delegatee back to vendor defaults, the same
      cadence-lapse capture mechanism recorded on the grounding strategies; (2)
      education and rule — the harness also rules mechanically (hooks,
      permissions); the never-imposed constraint of the diverged
      philosopher-ruler governs persons, not this delegatee; (3) turning bounded
      by the graft — the injected graph composes with, never replaces, the
      vendor habituation (kind-delegation's grafted constraints; the framing
      risk already on delegation-anthropic-claude's imported list). Recorded
      2026-07-08 interview."
  - question: Does /align-strategy itself run the tests this doctrine demands —
      internal consistency with the model of the good, and against plausible
      alternatives?
    answer: "No, as of this round. Step 2.2 checks only that serves resolves to a
      real virtue (referential integrity, validateGraph rule 8) and step 1.2 is
      keyword duplicate detection; there is no per-strategy
      doctrinal-consistency gate and no alternatives test at record time — those
      live only in strategy-philosophical-grounding's periodic rounds. The fix
      is retained as draft tactic tactic-align-strategy-alignment-tests. This
      round ran both tests only because the requirement text demanded them, and
      the consistency test caught a live failure mode: the session's stale
...
success_signal:
  observable: decisions trace to the graph — dialectic outputs cite node ids, a
    failed condition retires or re-derives a strategy, a calibration challenge
    moves a node
  sensor: owner review at office-hours
  threshold: each review cycle shows at least one decision that changed because a
    node changed
  is_proxy: true
attention: null
phase: null
