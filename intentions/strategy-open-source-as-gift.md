...
rationale: >-
  ...
  code on your behalf had any interest aligned with yours. The lesson:
  optimize for human freedom, not code freedom.


  Gifts are designed to transfer capability, not just tools. The budget
  tool's progressive disclosure — easy (analyze locally), medium (write a
  parser), hard (fork and host) — is structured so each level increases the
  user's autonomy rather than their dependency on the project.
