...
blocked_by: []
office_hours:
  reason: "/review-fix: code-review instrument unverified — Skill code-review
    cannot be used with Skill tool due to disable-model-invocation; its output
    was discarded per the unverified-instrument rule (not merged, not
    dispositioned, not credited); deviation=true, escalating rather than
    reporting a review that did not happen"
  since: 2026-08-01
  recommendation: >-
    # Next steps: `tactic-attention-tier-ranking` (PR #2997)


    ## What's actually broken


    Not this PR. The `/code-review` built-in ships with
    `disable-model-invocation`, so a subagent-context `Skill(code-review, 'max
    --fix')` call is rejected outright. `/security-review` has no such
    restriction, which is why it verified and ran normally on the same pass.
    This is a harness-level boundary, not diff-specific, and it will reject on
    every review-fix run until the invocation path changes.


    It is already diagnosed and tracked — do not file anything new:


    - `tactic-review-code-review-invocation-contract` (PR #3007, phase
    `implement`) replaces the Skill-tool call with a `claude -p` user-turn entry
    point. Its rationale records the measurement: **all 18** `Skill(code-review,
    'max --fix')` invocations across 07-27..07-31 were rejected, so the built-in
    has never actually run in dispatch.

    - That PR is itself parked:
    `tactic-hold-conflict-review-code-review-invocation-contract` holds a Lane-3
    merge conflict in `.claude/workflows/review-fix.js` against the just-landed
    instrument guard (`tactic-lane-instrument-substitution-guard`, `origin/main`
    778a1c94), whose CI-pinned greps in `test-review-fix-instrument.sh` assume
    the old shape. Its recommendation names the retarget path. Unblocking #3007
    is the systemic fix; this node is a symptom.


    Background on the instrument boundary:
    `.claude/skills/review-fix/references/schema-edge-cases-notes.md:67` (the
    #1172 / #2872 model split, reversed by #2887 on 07-18 to trust `/code-review
    --fix`). The guard that surfaced the failure landed today — that is why this
    is the first pass to park rather than silently pass.


    ## Resume options for this PR


    **(a) Run the missing pass by hand.** In an interactive session on this
    branch, run `/code-review max --fix` against #2997's diff, apply what it
    finds, commit, then clear the park and let the node proceed. Highest
    fidelity; costs one interactive sitting.


    **(b) Waive it.** The gap is quality-only (simplification, dead code,
    structure) — the security domain was covered: `/security-review` verified
    and ran clean, 8 domain finders produced 21 deduped findings, 2 real defects
    were found and fixed (162c5c32: `router.ts:265` cycle-throw emptying the
    selector; `goals.ts:84` tier fallback), 6 Required findings were
    adversarially refuted, 0 open CodeQL alerts. The change is an internal
    dispatch/graph-selection library, not a user-facing security boundary. Note
    also that every review-phase pass since 07-27 has the same missing
    instrument, so waiving here matches the coverage every other PR merged in
    that window actually got — it is not a special exception. If that reads as
    adequate for the risk level, record the waiver explicitly, clear the park,
    and re-run `/review-fix` or transition the node.


    Recommendation: (b) unless you want the quality pass on the router/goals
    edits specifically. (a) is cheap enough that it is a reasonable hedge if you
    are already in an interactive session.


    ## Audit trail


    Full disposition detail — every finding, every skeptic rationale, the
    refutations, and the advisory `cost` finding filed as draft tactic
    `tactic-select-targets-redundant-attention-resolve` — is in the single PR
    comment on #2997 marked `<!-- dispatch:review-fix -->`. Read that rather
    than re-deriving.
  session_type: other
pace_exempt: true
rounds: null
