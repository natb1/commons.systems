...
office_hours:
  reason: "/qa-fix: scope-deviation on item 10 (manual-tick sovereignty-floor
    override) — the disposition workflow's skeptics refuted the 'needs-human'
    framing, but the gated Opus fix-planner declined to author a fix, finding
    the triage's factual premise empirically false (the --manual branch only
    runs unsandboxed via the dispatch terminal command, so it will not misfire
    on routine ticks as feared) and refusing to reverse the node-authorized
    fail-closed change unilaterally as outside qa-fix's scope. Escalating to
    office-hours for a human authorization/sign-off call, not a code defect;
    recommendation comment attached names two low-risk suggested follow-ups (a
    stale contract-comment fix and a pgrep false-positive edge case)."
  since: 2026-08-03
  recommendation: >-
    # Best next steps — `tactic-graph-router-live-worker-read-robust` (PR #3010)


    ## What you are deciding


    One thing only: **does the PR's fail-closed override of the "human dispatch
    is sovereign" floor-of-one stand as shipped?**


    Not a bug hunt. All 9 script-verifiable QA checks pass; items 11 and 12 need
    nothing from you (11 resolved `already-satisfied`; 12's `## needs-main
    residue` already landed on the node body in `a3f8393a` on `origin/main` and
    auto-drains after merge).


    ## I already ran the verification the park asks for. Results:


    **1. `--manual` has exactly one caller, and it is unsandboxed.**


    - `nix/packages/dispatch.nix:33` — `exec "$TICK" --manual`. That is the only
    `--manual` invocation outside tests.

    - It reaches `dispatch-select-tick --manual` via
    `.claude/skills/dispatch-propagate/scripts/dispatch-tick:601`.

    - No skill, hook, workflow, or doc invokes `dispatch-select-tick --manual`
    or `dispatch-tick --manual` directly.


    **2. The probe behaves correctly in both directions on this host — measured,
    not assumed.**


    | context | visible PIDs | `pgrep -f 'claude daemon'` |

    |---|---|---|

    | sandboxed Bash | 5 (namespace-isolated) | no match → UNKNOWN → fail closed
    |

    | `dangerouslyDisableSandbox: true` | 65 | matches `.../bin/claude daemon
    run` (pid 1414079) → corroborated → normal |


    `systemctl --user is-active dispatch-claude-daemon.service` = `active`. The
    fix-planner's empirical claim holds.


    **3. The decisive fact the triage subagent missed:** `dispatch-select-tick`
    has *already* declared, since before this PR, that it must not be run
    sandboxed —
    `.claude/skills/dispatch-propagate/scripts/dispatch-select-tick:75-78`:


    > `Requires dangerouslyDisableSandbox: true — sub-scripts write tmp state,
    call gh, and query the local Claude daemon over a Unix socket.`


    A sandboxed `--manual` run was already an out-of-contract call. This PR does
    not create a new failure mode for it; it converts a *silent wrong answer*
    (fail open, spawn a duplicate worker onto an occupied worktree, no remedy)
    into a *loud correct one* (`concurrency-cap` + a stderr line naming the
    fix). That is the whole argument for item 10, and it is sound.


    ## Recommendation: sign off, item 10 settled


    Ratify item 10 as resolved and let the PR proceed to review → merge.
    Rationale, in order of weight:


    1. The only production invocation path is unsandboxed and verified working.

    2. The script's own header already forbids sandboxed invocation, so the
    fail-closed branch fires only on a call that was already contract-violating.

    3. The remedy is legible and one line of stderr away. The old behavior's
    remedy for a silent duplicate worker was: none.

    4. It matches this project's own repeated precedent for sandboxed liveness
    reads (`.claude/rules/sandbox.md` "claude agents --json" section — the `[]`
    ambiguity is documented there as a trap that must not be trusted).


    The `--manual` behavior change is covered by a real test using the real
    corroboration path (not a test-only override):
    `test-dispatch-select-tick.sh:1360` (Case 6) and Case 6b, which stubs
    `CLAUDE_AGENTS_PGREP_CMD` unreachable with a fake `claude` printing `[]`.


    ## Two concrete things to do while you're here


    **A. Fix the now-stale contract block in the same file — take this on THIS
    PR.** The PR updated the inline branch comment at
    `dispatch-select-tick:813-828` but not the file's own top-of-file contract,
    which still asserts the opposite.
    `.claude/skills/dispatch-propagate/scripts/dispatch-select-tick`:


    - line 58-62: *"per human-dispatch-is-sovereign, always launches its one
    highest-ranking-available node even at the MAX_WORKERS ceiling ... only the
    `--exhausted` floor (genuine rate-limit token exhaustion) still blocks it
    outright"*

    - line 66: *"The only hard floor is genuine token exhaustion."*


    Both are now false — daemon-UNKNOWN is a second hard floor. Item 10 is
    literally a question about this contract, so leaving the contract stating
    the pre-PR rule is the worst outcome. Two-line edit; no test changes.


    Nothing outside this file restates the floor-of-one — checked
    `.claude/skills/dispatch-propagate/reference.md` and the skill bodies. No
    wider doc sweep needed.


    **B. File a small follow-up tactic for the probe's false-corroboration mode
    — do not block this PR on it.**


    `pgrep -f` matches full command lines, and the sandbox's PID namespace
    *does* show the Bash tool's own wrapper shell. This was hit live during the
    recommendation research: a first sandboxed probe returned **corroborated
    (0)** purely because the command string contained the literal text `claude
    daemon`. Re-running with the pattern assembled from variables returned the
    correct **no-daemon (1)**.


    So a sandboxed caller whose command line happens to contain that substring —
    e.g. a debugging one-liner that greps for the daemon and runs a tick in the
    same compound command — gets a false corroboration and silently falls back
    to the *old* fail-open path. This is narrow, but it fails in the direction
    that actually hurts (duplicate worker, no diagnostic), which is exactly what
    this PR exists to eliminate.


    Cheap hardening for the follow-up, at `lib-claude-agents.sh:424`: anchor the
    pattern (`claude daemon run`, or match on the resolved binary path), or
    corroborate on the socket / `systemctl --user is-active
    dispatch-claude-daemon.service` instead of argv text. The test seam already
    exists (`CLAUDE_AGENTS_PGREP_CMD`, documented at
    `lib-claude-agents.sh:371-373`).


    ## One thing worth knowing, but not a blocker


    The triage worried about `--manual`. The path that will actually meet the
    new fail-closed exit is `graph-select-target --standalone`
    (`.claude/skills/dispatch-propagate/scripts/graph-select-target:405-417`),
    which this PR gives a new early `empty` + `live-read-unverified` exit.


    `--standalone` has **no in-repo call site** — its own header says so at
    lines 100-105 — and exists explicitly "for manual/emulated invocation."
    Emulated tick runs driven from a Claude session are exactly the sandboxed
    pattern. So this is where a sandboxed deferral will show up in practice.


    This looks acceptable and doesn't need to gate the PR: `--standalone` was
    *already* documented as not sovereign and *already* degrades to `empty` at
    the cap (header lines 84-92: "a `--standalone` caller is unattended, so it
    is ceiling-gated"). Fail-closed is consistent with its existing contract,
    unlike `--manual`, where the change is a genuine reversal. Its header also
    already carries the sandbox-off requirement (line 140). Worst case during a
    tick emulation you get `empty` plus a stderr line; re-run sandbox-off.


    ## If you want to re-check any of this yourself


    ```

    grep -rn -e "--manual" .claude/ nix/ --exclude-dir=.git | grep -v "/test-"

    sed -n '56,80p'
    .claude/skills/dispatch-propagate/scripts/dispatch-select-tick

    P1=cla; P2='ude dae'; P3=mon; pgrep -af "$P1$P2$P3"    # run BOTH sandboxed
    and sandbox-off

    ```

    Assemble the pattern from variables — a literal `pgrep -f 'claude daemon'`
    typed into a sandboxed Bash call self-matches and gives a false positive
    (that is finding B).
    ...
    If you accept the recommendation: apply the header fix (A), clear the
    `office_hours` park on `tactic-graph-router-live-worker-read-robust` via
    `packages/intentionsutil/scripts/clear-park`, and let review advance the
    phase.


    Two documented gotchas on that clear: pass `-C <worktree>` (a missing `-C`
    exits 0 while committing nothing and falsely reports "landed"), and sync so
    local `HEAD == origin/main` before clearing — a stale worktree re-clobbers
    `origin/main` on the park write.


    References:

    - #3010: https://github.com/natb1/commons.systems/pull/3010
  session_type: other
pace_exempt: true
rounds: null
attributes: {}
