...
blocked_by: []
office_hours:
  reason: "/qa-fix attempt 3: check-only pass found one real finding and the
    attempt cap is reached, so it parks rather than fixing. Item 3 — this PR's
    own cap raise (2 -> 3) left a stale cross-reference at
    .claude/skills/implement/SKILL.md:327, which still reads \"Cap at 2 fix
    attempts (mirroring qa-fix's CAP=2)\". The implement skill's own cap of 2 is
    unchanged and may be deliberate, but the parenthetical is now factually
    wrong. ATTEMPT_N=3 equals CAP=3, so plan_fix was false, no fixing pass was
    permitted, and no new attempt label is applied. Everything else passed: 7 of
    8 script-verifiable items PASS and the full suite is 3082/3082. Item 9
    (manual-tick launcher --standalone adoption) is unrelated to this park —
    already tracked as needs-main residue id 6 on the node body, confirmed
    intact at origin/main."
  since: 2026-07-25
  recommendation: >-
    ...
    Not ambiguity about the PR's substance. The fourth QA pass verified
    everything else clean — 7 of 8 script-verifiable items PASS, suite
    3082/3082, all 10 `--standalone` cases plus both new cap-default cases
    executed. One finding remains, and it is a side effect of the cap change
    this PR itself made.
    ...
    `.claude/skills/implement/SKILL.md:327` reads:


    > Cap at **2** fix attempts (mirroring `qa-fix`'s `CAP=2`).


    After this PR, `qa-fix`'s `CAP` is 3, so the parenthetical is false. The
    implement skill's *own* cap of 2 is untouched by this PR and is a separate
    number.
    ...
    This is a two-line edit, but which two lines depends on a judgment the
    automated lane should not make alone:


    - **Option A — raise implement's cap to 3 as well.** Keeps the two skills
    genuinely mirrored, and the rationale transfers: implement's fix lane also
    spends passes on whatever the verify block happens to fail on first. Costs
    one more `/implement-unit` on a stuck implement run.

    - **Option B — keep implement at 2, drop the parenthetical.** Treat the two
    caps as independent and delete the now-false cross-reference (e.g. "Cap at
    **2** fix attempts."). Cheapest, changes no behaviour.


    Option B is the smaller, safer change and is the recommended default; Option
    A is defensible if you want the mirroring to be real rather than incidental.


    ## To resolve


    1. Apply Option A or B at `.claude/skills/implement/SKILL.md:327`.

    2. If Option A, also update the implement skill's own cap prose so the
    number and the citation agree.

    3. Re-run
    `.claude/skills/dispatch-propagate/scripts/test-dispatch-scripts.sh` (expect
    3082/3082 — no test asserts implement's prose, so this is a regression
    check, not a proof of the edit).

    4. Clear the park with `packages/intentionsutil/scripts/clear-park
    tactic-graph-router-live-worker-visibility "<note>"` from a checkout at HEAD
    == origin/main, and let the chain re-QA.


    ## Not part of this park


    Item 9 — manual-tick launcher adoption of `graph-select-target --standalone
    --top <N>` — is a planned deferral already tracked as `## needs-main
    residue` id 6 on the node body. It is confirmed intact and does not block.


    ## Process note


    The Step 3.5 disposition Workflow was not run this pass. With
    `plan_fix=false`, every class it could assign to item 3 leads to the same
    terminal action, so it would have changed the park's wording and not its
    outcome.
pace_exempt: false
rounds: null
attributes: {}
