...
blocked_by: []
office_hours:
  reason: "Requirement ambiguity the author must rule on before Unit 1 can be
    planned: once /qa-fix stops writing a `## needs-main residue` body section,
    a WAIT-classed residue item has no recorded home in the record-time
    destination split, and the qa-phase `arm-wait` mint is keyed to the very
    hook this node deletes. Three recorded facts collide and no recorded ruling
    reconciles them. (1) The greenfield split is capped at two nodes per source
    (\"at most two per source, one carrying all machine-verifiable items, one
    carrying all author-required items\") and the sort mark is the required-core
    `owner` field, whose values are only human | ai | procedure (verified this
    round at packages/intentionsutil/src/schema.ts:6-9, required-core at
    schema.ts:979) — so a WAIT class has neither a third destination node nor a
    mark that can express it. (2) WAIT exists today ONLY in the interim
    per-bullet form (the `Verifiability: MACHINE|AUTHOR|WAIT` sub-line), whose
    own retirement note at
    .claude/skills/qa-fix/references/needs-main-followups.md:70-77 says it
    retires WITH the `## needs-main residue` body section itself and names THIS
    node as the trigger — while 44 live WAIT marks across 25 nodes in
    intentions/ (counted this round) are left by the 2026-08-06 non-backfill
    ruling (strategy clarification 210) to \"drain through the interim park
    path\" that Unit 1 removes. (3) That same clarification 210 assigns the WAIT
    producer wiring — the qa-phase mint of `tactic-wait-<source-id>` with its
    initial `attributes.wait_until` — to tactic-wait-calendar-release \"at
    needs-main-residue record time\". That node is at phase review with a
    codified 8-unit plan (waits.ts, wait-sweep.ts, validate-graph Rule 21, the
    router draft-candidate exclusion, wait-node-decide/list-recheckable-waits,
    arm-wait/release-wait, lib-wait-recheck.sh, dispatch-tick wiring) that
    builds the primitives but deliberately defers every skill-body call site
    (\"the flip itself is a skill-body edit an autonomous worker cannot commit;
    file it as a follow-up when this node reaches done\"), and its Sequencing
    note (intentions/tactic-wait-calendar-release.md:860-878) never names this
    node. So the mint's call site lands inside the exact /qa-fix Step 3.6 block
    this node rewrites, and neither node's record says which shape it writes
    into. CANDIDATE RESOLUTIONS, none ratified: (a) WAIT becomes a third
    per-source destination node — requires amending the at-most-two cap and
    choosing a mark, since `owner` cannot carry it; (b) a WAIT item routes to
    the `tactic-wait-<source-id>` hold node minted by `arm-wait`, with the
    machine-verifiable destination node `blocked_by` it, keeping the two-node
    cap intact and leaving the WAIT hold wholly tactic-wait-calendar-release's
    shape; (c) Unit 1 lands only after tactic-wait-calendar-release reaches done
    and that node absorbs the mint call-site flip, this node's plan stating the
    ordering explicitly (the precedent clarification 176 set for the sort
    tactic). The same ruling fixes the fate of the 44 live WAIT marks: drain in
    place under this node's migration plan, or migrate. Not planner-resolvable —
    each candidate either amends a recorded clarification or reassigns work the
    author has already assigned by name across two nodes, and strategy condition
    4 keeps persistent-layer substance human-decided. Round accounting: this was
    a tactic-target (per-node) round, which never writes the serving strategy,
    so the drift phase's three immaterial observations were NOT landed as
    strategy clarifications; they are instead recorded verbatim in this node's
    own body under \"## 2026-08-19 /align-tactics round notes (parked)\" so a
    fresh session need not re-derive them. Recommend: rule the WAIT-destination
    question in an /align interview on strategy-graph-native-dispatch, then
    re-run /align-tactics tactic-mainqa-record-time-routing."
  since: 2026-08-19
  recommendation: "Author decision needed, ~20 min sitting. Open /align on
    strategy-graph-native-dispatch and rule the WAIT-destination question stated
    in the reason: pick (a) a third per-source destination node, (b) route the
    WAIT item to the `tactic-wait-<source-id>` hold node minted by `arm-wait`
    with the machine-verifiable destination node `blocked_by` it, or (c)
    sequence Unit 1 behind tactic-wait-calendar-release reaching done and let
    that node absorb the mint call-site flip. Candidate (b) preserves the
    recorded at-most-two cap and needs no clarification amendment, so it is the
    cheapest to ratify; (c) is the safest if the author wants the sibling's 8
    units to land untouched first. The same ruling must also say what happens to
    the 44 live `Verifiability: WAIT` marks across 25 nodes — drain in place or
    migrate. Then re-run `/align-tactics tactic-mainqa-record-time-routing`; the
    next round needs no re-derivation, since the corrected anchors, the
    superseded/blocked sibling map, and the two stale-unit corrections are all
    recorded in this node's body under the 2026-08-19 round-notes section.
    Nothing else blocks this node: the core premise still holds at origin/main
    and the reuse precedent for minting nodes from a phase skill is already
    identified."
  session_type: other
pace_exempt: false
rounds: null
