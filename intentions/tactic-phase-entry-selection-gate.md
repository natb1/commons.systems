...
- **Exit routing:**
  ...
  - `13` — not reachable at this phase (`SCOPE_CHAINED_PHASES` is `fix`/`qa`/`review` only, `check-node-selection.ts:61`). Treat it as a mechanical error: report and stop.
  - any other non-zero — mechanical error (unresolvable project root, failed fetch, malformed store): report and stop. A stale selection is **not** an `office_hours` park and **not** a defect; a mechanical error is neither either — it is a broken environment to report, per `.claude/rules/code-style.md`.
