---
id: tradition-aristotle
kind: tradition
statement: "Aristotelian ethics — the graph's structural chassis: virtue as
  hexis, tension as the mean, phronesis at the root"
owner: human
status: delegated
parent: null
rationale: "The deepest structural inheritance in the graph. kind-virtue's
  'disposition held and exercised, never completed' is nearly a direct
  translation of Aristotle's definition of virtue as a hexis expressed in
  energeia; the tension machinery ('balanced, not summed; maximizing one against
  its partner is failure') is the doctrine of the mean rendered as graph
  topology; and virtue-philosophical-mobility is a phronesis-shaped root —
  including its darkest clause — though the graph's capture doctrine was amended
  2026-07-07 (tradition-plato, chunk-1 reading): capture MISDIRECTS the sensor
  rather than destroys it, and Aristotle's reading here — vice destroys the
  archē of action, the corrupted agent cannot perceive the corruption — is the
  stronger corruption thesis, now a provisional divergence held open for the NE
  VI reading (see diverged, added 2026-07-09). The attachment ledger's
  seriousness about externals is his too: eudaimonia needs external goods,
  against Stoic sufficiency. Structure is Aristotle's; method credit for the
  dialectic itself goes to tradition-plato — the two partition cleanly except at
  the capture-doctrine locus, where a provisional Plato-over-Aristotle choice is
  recorded (2026-07-09, on tradition-plato) pending the NE VI reading. Recorded
  2026-07-04 interview."
reading: null
gap: null
serves: []
recovers: []
clarifications:
  - question: What did the chunk-2 reading (NE II.1-7) ratify about habituation?
    answer: "Ratified at chunk 2 (II.1, II.4). 1103a14-26 assigns virtue of
      character to habit, not teaching. The author's articulation: a teacher is
      required for good habituation — 'otherwise no teacher would be needed, but
      everyone would be born a good or a bad craftsman' (1103b12) — but
      transmits no knowledge that constitutes the virtue; as with Republic
      518b-d's periagoge (chunk 1), the teacher turns the practice, directing
      which acts get repeated; the doing forms the state. Virtue of character is
      in this sense a social act: the same activity ruins or realizes the state
      depending on guidance (the harp, 1103b8-12), and the right education is to
      delight and be pained by the right things from youth (1104b11-13, citing
      Plato). II.4 completes the ratification: the circle — doing just acts
      before being just — is entered by chance or at another's direction
      (1105a22-23; the author's guidance reading, vindicated by the text), while
      the doer is just only when acting knowingly, choosing the acts for their
      own sakes, from a firm and unchanging state; knowledge counts for little
      or nothing where the other two count for everything (1105b1-3), and those
      who take refuge in arguments are patients who hear the doctor and follow
      none of the prescription (1105b12-18). The sign of the formed state is the
      pleasure supervening on the act (1104b3-8) — distinguishing virtuous
      action from virtuous character. Recorded 2026-07-13 /reading-review chunk
      2."
  - question: What did reading chunk 5 (NE VI + III.4) resolve on this record?
    answer: "Nothing yet, by design. The sitting completed 2026-07-17 with draft
      verdicts touching this record — retire-and-adopt on the provisional
      vice-corrupts divergence (with tradition-plato's provisional chosen_over
      in lockstep), a III.4 both/and adopted entry (capture corrupts appearance,
      not wishing), the spoudaios self-certification amendment to the
      fallibilism divergence, and the chunk-5 vocabulary/quote retention lists —
      but the author deferred the entire slate en bloc (2026-07-18: 'all of
      these outcomes sound correct but there is still too much platonic glare to
      ratify outright'). The full ledger is quarantined on
      tactic-dialog-review-aristotle-phronesis, the ratification gate; this
      record changes only when that dialog session ratifies, amends, or
      re-defers each item. Recorded 2026-07-18 /reading-review chunk 5."
tooling_goals: []
success_signal: null
attention: null
phase: null
execution: null
validates: []
blocked_by: []
office_hours: null
pace_exempt: false
rounds: null
attributes:
  origin: chosen
  adopted:
    - virtue as hexis, exercised as energeia (NE II.5-6) → kind-virtue's
      'disposition held and exercised, never completed', 'exercised through its
      constituent virtues' — ratified 2026-07-13 chunk-2 reading (II.5 genus
      argument personally verified; see kind-virtue's chunk-2 clarification)
    - "the mean's FORM — virtue as a target ruined from two directions, by
      excess and deficiency (NE II.2, II.6) → the shape tension_with borrows:
      balanced, not summed, maximizing one term against its partner is failure.
      Requalified 2026-07-13 chunk-2 reading: the doctrine itself is
      intra-virtue and is NOT the source of the inter-virtue balance — see
      diverged"
    - phronesis as non-delegable discernment, and vice corrupting the archē (NE
      VI) → virtue-philosophical-mobility; the companion claim 'capture corrupts
      the sensor' was amended 2026-07-07 to 'capture misdirects the sensor'
      (tradition-plato, chunk-1 reading) — the corruption reading is now the
      provisional divergence below, pending the NE VI chunk
    - external goods matter to eudaimonia (NE I.8-10) → the delegation ledger;
      attachments managed, not renounced (see tradition-stoicism, declined)
    - precision only as far as the subject admits (NE I.3) → the apex question
      held open rather than forced to system (clarifications on both root
      virtues, 2026-07-04)
    - sōphrosynē — temperance as ordered appetite (NE III.10-12) →
      virtue-temperance, rooted 2026-07-04; previously the largest Aristotelian
      gap in the root forest
    - "liberality (NE IV.1) → strategy-open-source-as-gift: giving that
      transfers capability is giving well"
    - schole — leisure as the space of the best activities (NE X.7, Politics
      VIII) and philia (NE VIII-IX) → strategy-tabletop-storytelling
    - "the human as political animal (Politics I.2) → community as calibration:
      strategy-join-existing-practice, the practitioner tier"
  diverged:
    - eudaimonia as single apex — the root layer is a forest; the tacit-apex
      question is held open as a standing review trigger, not resolved in
      Aristotle's favor
    - habituation as the whole method — the graph adds explicit sensors,
      signals, and external calibration machinery Aristotle lacks; modern
      fallibilism grafted onto his psychology
    - the polis as virtue's precondition — the graph is practitioner-individual;
      community enters as calibration and gift-recipient, not as constitution
    - "vice destroys the archē (the corruption thesis, NE VI) — vs the graph's
      amended account that capture misdirects the sensor and agency is never
      lost (tradition-plato, 2026-07-07 chunk-1 reading). Provisional divergence
      recorded 2026-07-09 AHEAD of the Aristotle reading: the NE VI chunk must
      resolve whether Aristotle's claim is truly the stronger corruption thesis
      or compatible with misdirection; this entry retires or hardens accordingly
      (matching provisional chosen_over on tradition-plato)"
    - "the mean as intra-virtue (NE II.6: a mesotes between two vices, one of
      excess and one of deficiency, within one virtue's field of feeling and
      action, relative to us, fixed by the reason by which the phronimos would
      fix it, 1106b36-1107a2; II.7's table individuates virtues by field and
      balances none against another) — vs the graph's tension_with, which
      relocates balance BETWEEN sibling virtues as a cross-guard (each sibling's
      excess falls in the other's field). A graph-native device that borrows the
      mean's shape, not Aristotle's doctrine; recorded per the chunk-2 reading,
      2026-07-13 (author verified against II.7 directly)"
  chosen_over:
    - "over tradition-kant at virtue-respect-for-persons' practice layer: how to
      respect well (when to promote, what counts as imposing) is phronesis, not
      derivable from the categorical imperative"
    - "over tradition-plato at kind-virtue: virtue is acquired by habituation
      and assumes drift and relapse, against virtue-as-knowledge"
    - "over tradition-stoicism at the attachment ledger: external goods matter;
      attachments are managed, not renounced"
  texts:
    - Nicomachean Ethics I.3, I.8-10, II.1-7, III.10-12, IV.1, VI, VIII-IX, X.7
    - Politics I.2, VIII
  review_trigger: any strategy-philosophical-grounding round touching
    structure-level doctrine — hexis, tension semantics, phronesis, the apex
    question
  last_assessed: 2026-07-13
---
# Aristotelian ethics — the graph's structural chassis: virtue as hexis, tension as the mean, phronesis at the root
