 ...
 * `toUsageSample`/`toIssueSample` parsers are NOT reused: they require a
 * Firestore `.toDate()` Timestamp and a `memberEmails` field the serialized
 * snapshot intentionally drops, so they would reject every serialized point and
 * silently truncate the series to one point.
 */
