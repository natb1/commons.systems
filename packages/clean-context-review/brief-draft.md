...
**The probes** (`author-questions`, `probe-or-node`). A probe is a question you need the author to answer before this draft's recommendation can be grounded, and whose answer is not itself a disposition. Raise one on this node only where all three limbs hold, and write none where any fails: the record does not answer it and you have looked, and your `why` names the locus you read and what that locus leaves open; the answer would move a recommendation on this node, and your `discharges` names which; and the answer is not itself a disposition, which you decide by what the author's response would do once it is given and never by how the question is worded, since any question can be worded either way and the wording is the asker's to choose. An open question has three homes and this rule sends it to one of them: it is an **option** where you hold a candidate answer to this node's question viable, whether or not you would recommend it; a **probe** where you hold no candidate you can recommend among without knowing what the author intends, and the response would be consumed whole by moving a recommendation here; and a **node** where the response would have to stand — be enforced by the record, be cited by another node, project a rule, ground work, be read by a session that never saw the question, or be delegated or deferred. The **ruling test**: if it would make sense for the author to respond "delegated, do not ask me again" or "deferred, I will return to it", the question is a node, because only a disposition carries a class, and a probe's answer can be neither delegated nor deferred. The **scope test**: a question whose answer would move recommendations on more than one node, or would bind nodes not yet written, is doctrine reaching below it, which is a node's work and not a probe's, and every node it would have moved enters it in `depends`. The **survival test**: if a later sitting, a reading, a projection or a reconciliation would need the response on its own account after the recommendation it moved has been ruled on, it is a node, since a probe dies with the dialogue and what survives of it is the author's words and the rationale that quotes them. Neither the absence nor the presence of the author's words is evidence either way: most of this record was minted without quoting them, so silence is no evidence that no disposition stands behind a node, and a quotation is no evidence that a question is a probe. A question you cannot classify is raised as a probe, because that is the reversible error — a probe whose answer turns out to need to stand is promoted to a node then, with the author's words in hand to found it, where a node can only be demoted by a prune the author must rule on — and a question whose response would both move this node's recommendation and stand is a node, because a node can carry both and a probe neither, and it goes to validation 15's merge finding above and never here. **Finding no probe is a complete answer** and not a step left undone: an empty array is what a draft you can ground returns, and a probe raised to show diligence is the paper doubt this record refuses.

**There is no cap on how many probes stand open on a node.** The author struck it at `words/2026-09-08/32`: an arbitrary cutoff buys nothing, and its worst effect is variance in encoding, the same content standing in the `probes` field below the number and in prose above it. Probes are bounded without a cap because they block on the author rather than recursing, and the remedy for a long list is to refine the disposition until it can be confirmed, never to stop asking. So do not report a count as a finding. The `#### Probes` block in the node section below lists every probe the node already carries, open and discharged, for a different reason: a reader shown no probe re-raises what the record has already asked. Read them before you raise one.
