...
**The cap is three open probes on one node**, a probe compounding two questions counting as the two it compounds. Check it on the node you read: the `#### Probes` block in the node section below lists every probe the node already carries, open and discharged, with the count of the open ones against the cap, so count from there and add the ones you raise, and report an excess as a finding naming the node and the probes, so that the movement discharges or withdraws one before it raises another. The cap binds the movement and is checked by this reading; it is not enforced by the parser, so expect no parse failure from it.
