...
This brief is the whole of what you read. It carries the validations you run, quoted below from `frontier-consistency`; the judged set, whole; and its neighbourhood, each node by what it answers. Read no node file whole and read no file this brief does not point you to: the thirteen nodes an earlier form of this brief told you to read in full (`frontier-consistency`, `clean-context-review`, `alignment-order`, `recording`, `viable-options`, `authority`, `unanswered`, `dialogue`, `node`, `evaluation`, `materialization`, `session-context`, `delegation`) are all carried below by what they answer, and reading them whole is what overflowed a reader on 2026-09-07. Open a node's file only at a locus a finding needs — one `## Answer` section, one option subsection, one ledger entry — and never to re-read what stands here. Read this brief in pieces of no more than 600 lines per Read call, from the first line to the last, all of it before you write any finding, since the survey compares nodes; if you cannot finish it, say in `summary` which line ranges you left unread.
