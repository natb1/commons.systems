...
16. **Independence** (`probe-or-node`). A node whose `under` names a node that itself carries a `stage`, whose only possible answer is a reading of that parent's answer, whose facts would repeat the parent's, and which would be pruned the moment the parent's recommendation moved, is a probe on the parent and not a child of it: its question can be put as "on the parent, which did you intend", and that is a probe's question. Report it as a finding naming both nodes and carrying, in the `proposal`, the probe the child would become, with the `asks`, the `why` and the `discharges` it would take and the note that its options are struck as options that were never candidates and that any words of the author's on it move to the parent's `## Disposition`; and never as a probe raised, since re-encoding a node is the main thread's and not this reading's. Kind: `decomposition`, the schema's nearest — the child is a fragment of its parent, and a fold is what is proposed. A reading (`form: reading`) is exempt by construction: its answer is a standing relation between a tradition and the node above it, which stands whatever the parent recommends.
