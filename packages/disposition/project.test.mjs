...
test("checkRules reports a missing, a stale and an orphaned rule file, and writes nothing", async () => {
  const dir = await freshTmpDir("project-rules-check-");

  // Nothing projected yet: the one global-tier node's file is missing.
  const empty = await checkRules(VALID_GRAPH, dir);
  assert.equal(empty.ok, false);
  assert.equal(empty.missing.length, 1);
  assert.ok(empty.missing[0].startsWith("root-b.md: no such file"), empty.missing[0]);
  assert.deepEqual(await readdir(dir), [], "a check writes nothing");

  // Projected, then the node amended under it: stale, not missing.
  await writeRules(VALID_GRAPH, dir);
  assert.equal((await checkRules(VALID_GRAPH, dir)).ok, true, "current right after a projection");
  const file = join(dir, "root-b.md");
  await writeFile(file, `${await readFile(file, "utf8")}\nAn edit the graph does not carry.\n`);
  const stale = await checkRules(VALID_GRAPH, dir);
  assert.equal(stale.ok, false);
  assert.deepEqual(stale.missing, []);
  assert.equal(stale.stale.length, 1);
  assert.ok(stale.stale[0].startsWith("root-b.md: stale"), stale.stale[0]);

  // A file carrying the notice that no global-tier node projects: orphaned.
  await writeRules(VALID_GRAPH, dir);
  const notice = "Generated by packages/disposition/project.mjs --rules; do not edit.";
  await writeFile(join(dir, "gone.md"), `# Gone\n> Projected from example.test/main/gone. ${notice}\n\nText.\n`);
  await writeFile(join(dir, "hand-written.md"), "# Not a projection\n\nLeft alone.\n");
  const orphan = await checkRules(VALID_GRAPH, dir);
  assert.equal(orphan.ok, false);
  assert.deepEqual(orphan.stale, []);
  assert.equal(orphan.extra.length, 1, `hand-written files are not reported: ${orphan.extra.join(" | ")}`);
  assert.ok(orphan.extra[0].startsWith("gone.md:"), orphan.extra[0]);
});
