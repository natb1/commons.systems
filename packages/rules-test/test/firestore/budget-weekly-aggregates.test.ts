...
describe("budget weekly-aggregates", () => {
  ...
  beforeEach(async () => {
    await adminSetDoc(env, `budget/${ENV}/weekly-aggregates/agg1`, {
  ...
  it("allows member to read", async () => {
    ...
    await assertSucceeds(
      getDoc(doc(db, `budget/${ENV}/weekly-aggregates/agg1`)),
    );
  });
  ...
  it("denies write", async () => {
    ...
    const db = ctx.firestore();
    await assertFails(
      setDoc(doc(db, `budget/${ENV}/weekly-aggregates/agg1`), {
        memberEmails: ["member@test.com"],
        total: 200,
      }),
    );
