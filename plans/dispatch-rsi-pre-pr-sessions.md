...
**Nothing gates the first PR.** Per Revision 5 and Revision 6 of the main plan,
the node-lane parking session was withdrawn (zero mergeable node-lane PRs, and
no path to a non-zero set while the sentinel holds) and no pre-pause baseline is
usable (the ladder was not working during that period, so a broken-before
against fixed-after comparison is confounded). Bundle 1 starts cold.

Every session below gates Bundle 3 or Bundle 5 — fourth and fifth in the
recommended order.
...
|---|---|---|---|---|---|
| 1 | code-review lock design | PR6 | Bundle 3 (4th) | none | any time; see the sequencing note |
| 2 | cache-preserving context | PR7 | Bundle 3 (4th) | none *in code*; Bundle 4 to clear the graph edge | during the window |
| 3 | observation masking | PR7 | Bundle 3 (4th) | none for the cost half | split — see below |
| 4 | band-derivation ratification | PR10 | Bundle 5 (5th) | none | any time |
| 5 | tradition: agentic engineering | PR11 | Bundle 5 (5th) | none | any time |
| 6 | fan-out and model routing | PR11 | Bundle 5 (5th) | none (`blocked_by: []`) | during the window |

Three are `/office-hours` sittings (1, 4, 5) and three are measurement sessions
(2, 3, 6). Only one has a real merge prerequisite, and it is weaker than it
looks.
...
including now. Running them early is free and removes them from the critical
path — with the one deliberate exception of session 1, whose ordering against
PR6 Unit 1 is a real judgment call rather than a scheduling detail.
