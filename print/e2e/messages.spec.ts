...
test.describe("messages", () => {
  ...
  test("home page displays seeded messages", async ({ page }) => {
    ...
    await expect(messages.nth(0)).toContainText("Welcome to print");
    await expect(messages.nth(1)).toContainText("Hello from Print");
  });
  ...
  test("messages appear in chronological order", async ({ page }) => {
    ...
    expect(texts[0]).toContain("Welcome to print");
    expect(texts[1]).toContain("Hello from Print");
  });
