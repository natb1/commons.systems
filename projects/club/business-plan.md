...
*Working draft — v0.3.5 (post-adversarial-review reframe + interview clarifications + free-hosting pilot re-sequencing, Aug 27–28 2026; §5/§6 figures moved onto the single-source model and the cost base rebuilt from graded components, Aug 29 2026)*
...
**v0.3.5 (Aug 29, later):** **the cost base says what it knows and what it is guessing.** Every cost is declared once — with the stream it serves, what its amount scales with, an evidence grade (A contracted · B observed · C benchmark · D assumed) and, where it is an estimate rather than a quote, the band it sits in — and `model/evidence.md` publishes the whole registry together with a ranking of what each estimate is worth if it is wrong. Two lines that were residuals become quantities. **Operations** is summed from utilities, insurance, software and repairs plus card fees and marketing on revenue, then compared with the $65K / $62K §6 stated rather than solved out of them; it agrees within 3% at both sites. **Occupancy** is base rent plus a labeled NNN component. The check that used to guard the operations split is deleted because it could not fail — the split was defined so that its fixed base landed identically at both sites, whatever the two totals were — and a reconciliation that can fail replaced it. §6 moves as a consequence, not as a new assumption: operations $65K / $62K → <!--m:opsLi-->$67K<!--/m--> / <!--m:opsSn-->$64K<!--/m-->, owner comp −$13K → <!--m:compLi-->−$15K<!--/m--> (LI) and −$37K → <!--m:compSn-->−$38K<!--/m--> (SN/HT). The ranking's top two entries are the NNN residual and insurance, both grade D and both worth more than any operating lever the plan exposes — they are open-questions §C #20a.
...
### Decision log (operator interview, Aug 27 2026; D16 revised Aug 28; D17 from the model, Aug 29)
...
| D17 | Capital asserted, not derived (model finding, Aug 29 2026) | **Required capital becomes a derived output** of site geometry, deal terms and a monthly ramp, declared in a uses registry carrying the same provenance obligations as the cost registry, and the owner's bar charges its return on the capital that registry derives rather than on a separate slider. **The cap breach this surfaces is reported, not engineered away** (§6, risk 4): required capital exceeds D7's cap at both sites, and the plan's own grant-free cash-to-open range has been contradicting that cap inside the same model all along. Working capital stops being an input — §6's band survives as the stated basis the built figure is reconciled against, and that reconciliation shows the band is a gate-case figure. D7's build-out number becomes a **scope cap** checked against derived fit-out rather than the fit-out budget. Full reading: `model/evidence.md`; the standing design proposal that specified this work is superseded by its implementation and deleted |
...
Common cost base: café cost of sale <!--m:cafeCogsPct-->32%<!--/m--> (a <!--m:cafeMarginPct-->68%<!--/m--> contribution margin), catering 40%, consignment 60%; **labor at a living wage: <!--m:laborPlanK-->$120K<!--/m--> at the ~$23/hr this table assumed**, covering café day + hosted event nights, net of the owner's ~30 floor hours/wk (D5, D6) — priced bottom-up, the same <!--m:staffHrsPerYr-->5,217<!--/m-->-hour grid costs <!--m:laborRungK-->$121K<!--/m--> at a properly loaded MIT living wage (see the rungs below); **operations <!--m:opsSn-->$64K<!--/m-->–<!--m:opsLi-->$67K<!--/m-->**, built from named components rather than quoted as a total — a <!--m:fixedOpsK-->$47.0K<!--/m--> fixed base (utilities, insurance, software, repairs & misc) plus <!--m:varOpsPct-->5.5%<!--/m--> of gross revenue (card fees ~3% + marketing ~2.5%), each component carrying an evidence grade and, where it is an estimate rather than a quote, the band it is estimated within (`model/evidence.md`); built that way the line comes to <!--m:opsBuiltLi-->$66.8K<!--/m--> (LI) and <!--m:opsBuiltSn-->$63.5K<!--/m--> (SN/HT) against the <!--m:opsStatedLi-->$65K<!--/m--> and <!--m:opsStatedSn-->$62K<!--/m--> this section carried through v0.3.4 — agreement within <!--m:opsDeltaPctLi-->2.7%<!--/m--> and <!--m:opsDeltaPctSn-->2.4%<!--/m-->, and still the replacement for v0.2.1's $38K that review F1 asked for; **occupancy** is built the same way — base rent at the site's square footage and rate, plus a labeled NNN component (CAM, taxes, building insurance) whose implied rate per square foot is checked against the market band instead of sitting unnamed inside the total; **commons/books line <!--m:commonsK-->$6K<!--/m-->** (D1/D3 — the budgeted philanthropy).
...
Operations is one figure per site in the plan. This row does not use it: the line is built from named components — a $47.0K fixed base (utilities, insurance, software, repairs & misc, each carrying an evidence grade and a band) plus 5.5% of gross revenue for card fees and marketing. Built that way it comes to $66.8K at Little Italy against §6's stated $65K (+$1.8K, +2.7%) and $63.5K at SN/HT against $62K (+$1.5K, +2.4%) — close enough to say the two agree, far enough apart that a component drifting would show. `model/evidence.md` carries the grades, the bands, and what each one is worth if it is wrong. Cost of sale is rebuilt from the per-stream margins (café 68% contribution, catering 60%, consignment 40%, rooms 94%) rather than carried as a lump, so the column sums to its own owner-comp row.
<!-- model:end pro-forma -->

*Reconciliation with the figures this table carried through v0.3.3, all of them consequences of the model rather than new assumptions: **SN/HT revenue** reads <!--m:revenueSn-->$300K<!--/m--> against a printed $301K — its five streams summed, with the rounding gone. **Cost of sale** reads <!--m:cogsLi-->$104K<!--/m--> (LI) against a printed $104K, because it is now built per stream at each stream's own margin instead of quoted as a 33–40% band. **Operations** reads <!--m:opsLi-->$67K<!--/m--> / <!--m:opsSn-->$64K<!--/m--> against a printed <!--m:opsStatedLi-->$65K<!--/m--> / <!--m:opsStatedSn-->$62K<!--/m-->, because the line is now summed from its own components instead of quoted as a total. **Owner compensation** moves with all three, and the two columns are worth tracing separately, because they did not fail the same way. Little Italy's v0.3.3 column tied exactly — 360 − 104 − 120 − 65 − 6 − 78 = −13 — and the model now reads <!--m:compLiExact-->−$15.2K<!--/m-->: <!--m:opsDeltaLi-->+$1.8K<!--/m--> of the move is the built operations line running above the $65K that column used, and the remainder is the rounding in that column's revenue and cost-of-sale lines. SN/HT's column did not tie: its own arithmetic (301 − 85 − 120 − 62 − 6 − 64) is −$36K, so the −$35K printed above it was a slip. The model reads <!--m:compSnExact-->−$38.3K<!--/m-->, of which <!--m:opsDeltaSn-->+$1.5K<!--/m--> is the built operations line and the rest is the rounding in that column's revenue and cost-of-sale lines.*

The same profit and loss, grouped by stream rather than by cost nature — what each part of the business earns before the costs no single stream owns. The books row is negative by design: D3 makes the book wall a draw rather than a revenue line, and the <!--m:commonsK-->$6K<!--/m--> commons budget sits against it as budgeted philanthropy, not as a stream that failed.

<!-- model:begin stream-margins -->
*Generated from `model/model.mjs` — edit the model, then `node model/render.mjs`.*

| Little Italy at plan marks | Revenue | Cost the stream incurs | Margin | Margin % |
|---|---|---|---|---|
| Café | $303K | $107K | +$197K | 64.8% |
| Rooms | $51K | $5K | +$46K | 91.0% |
| Books | $6K | $10K | −$4K | −63.0% |
| Cross-cutting | — | $255K | −$255K | — |
| **Owner compensation** | | | **−$16K** | |

*A cost is charged to a stream only where the evidence puts it there: each stream's own cost of sale, its own card fees at 3.0% of its own revenue, and the $6K commons budget the books wall exists to fund. The cross-cutting row is deliberately left whole. The three defensible ways to divide it — revenue share, floor area, staffed hours — give three different answers, so the explorer makes the basis a visible control instead of the model picking one. Labor here is the default Wages rung ($121K) rather than §6's $120K line, which is why this table's owner comp sits $1.1K below the pro forma's.*
<!-- model:end stream-margins -->
...
The Lean row is **not survivable** — it triggers the stop rule. Neither, on the model's own arithmetic, is the base case carried by a reserve: opening cost is now derived rather than asserted (§6 below), and at Little Italy's marks it consumes the whole <!--m:ventureCashK-->$150K<!--/m--> of venture cash and more, so nothing is left to absorb operating losses over the <!--m:runwayMonths-->24<!--/m-->-month runway and the bail line is <!--m:bailLineK-->$0K<!--/m-->/yr. The base case used to read as a slow bleed a reserve could carry past an honest verdict; there is no reserve behind it, which makes it a G2 finding rather than a P&L one.
...
Venture-available cash is **under <!--m:ventureCashK-->$150K<!--/m-->**, grant-free, covering **everything**: fit-out, deposits, FF&E, working capital, and Year-1 ramp. That sentence is now checkable, and it does not check out. Opening cost is derived from site geometry and deal terms rather than asserted — every line declared with a driver, an evidence grade and a band, the way the cost registry is (`model/evidence.md`). At Little Italy's marks on the lease path required capital is **<!--m:requiredCapitalLiK-->$218.9K<!--/m-->**, or **<!--m:headroomLiK-->−$68.9K<!--/m-->** against the cap; at SN/HT it is **<!--m:requiredCapitalSnK-->$280.9K<!--/m-->** (**<!--m:headroomSnK-->−$130.9K<!--/m-->**). The figure this replaces carried fit-out and working capital only — two of the five lines this paragraph names. Deposit (<!--m:depositLiK-->$13.0K<!--/m--> at LI) and FF&E (<!--m:ffeLiK-->$14.0K<!--/m-->) were named here and absent from the model; the occupancy owed during construction, the payroll spent hiring and training before opening, licensing and professional fees, opening inventory and contingency were absent from both.

**The plan has been contradicting itself inside the same model.** The grant-free cash-to-open range it carries — <!--m:equityRangeK-->$180–300K<!--/m-->, the range the owner's bar has been charging its return on all along — sits entirely above D7's cap. Required capital lands inside that range and outside that cap. Nothing flagged it because the bar and the cash tile were separate sliders that never spoke to each other. Per D17, the breach is reported, not engineered away: no input was moved to make the plan look consistent. Consequences, stated plainly:

- **Lease path only** from own cash — and, at these marks, not that either without concessions. The build-out figure is now a **scope cap** of **~<!--m:buildCapK-->$85K<!--/m-->** rather than a budget (phased fit-out, used FF&E, landlord TI negotiated hard — #17): the derived fit-out is <!--m:fitoutLiK-->$85.0K<!--/m--> at Little Italy, which is where the cap came from, and <!--m:fitoutSnK-->$102.0K<!--/m--> at SN/HT — a <!--m:scopeOverrunSnK-->$17.0K<!--/m--> overrun the site comparison never priced against the cheaper floor's <!--m:snFloorAdvantageK-->+$14K<!--/m-->/yr.
- **Working capital is derived, and §6's ~<!--m:wcBandK-->$50–65K<!--/m--> turns out to be a gate-case figure.** At the base case the ramp never turns cash-flow positive inside the model's horizon — stabilized comp is negative, so there is no trough and no finite requirement; what the model reports is a runway choice, <!--m:runwayReserveK-->$32.5K<!--/m--> to fund <!--m:runwayMonths-->24<!--/m--> months at the stabilized burn. The base case at SN/HT does have a trough — <!--m:wcBuiltSnK-->$78.8K<!--/m-->, well past the band — and it is at the *gate* marks, not the base ones, that the built figure meets the band §6 states: squarely at SN/HT, and at Little Italy only once abatement runs past the construction months into the operating ones. The four-row reconciliation is in `model/evidence.md`.
- The **buy path requires SBLP + grants/TI to exist at all** (10% contribution + closing + FF&E exceeds available cash alone). That is consistent with D8: buy is the acceleration case. One line of it does not reproduce: §6 states buy occupancy as the lease line plus a small premium — <!--m:statedBuyOccupancyK-->$66K<!--/m--> — while a loan at the model's declared <!--m:loanPricePsf-->$105.0/sf<!--/m--> — debt service, property tax, owner's insurance and a structural reserve — totals <!--m:builtBuyOccupancyK-->$39.4K<!--/m--> (<!--m:buyOccupancyDeltaK-->−$26.6K<!--/m-->). The price that would reproduce §6 is <!--m:buyReconcilePricePsf-->$190.9/sf<!--/m-->, outside the defensible <!--m:buyPriceBandPsf-->$60–150/sf<!--/m--> band for these submarkets. Occupancy stays on §6's stated basis pending a real price, so owner comp is unmoved on either path; the failed reconciliation is reported in `model/evidence.md` rather than forced.
...
4. **Cash constraint — on the model's own arithmetic the stop condition is already met.** This risk used to read: if lease-path all-in costs can't fit ~<!--m:ventureCashK-->$150K<!--/m--> after TI negotiation, that is a G2 stop, not a stretch. Those costs are now computed rather than asserted, and at the plan's defaults they do not fit — <!--m:requiredCapitalLiK-->$218.9K<!--/m--> at Little Italy (<!--m:headroomLiK-->−$68.9K<!--/m-->) and <!--m:requiredCapitalSnK-->$280.9K<!--/m--> at SN/HT (<!--m:headroomSnK-->−$130.9K<!--/m-->), before any TI. So G2 cannot be cleared on the terms as written: it needs concessions, a smaller scope, grant capital, or some combination, and the capital ranking in `model/evidence.md` says the largest lever is <!--m:topCapitalSwingLabel-->Fit-out (net of TI and grants)<!--/m-->, worth <!--m:topCapitalSwingK-->$57.5K<!--/m--> across its <!--m:topCapitalSwingBand-->$25–45/sf<!--/m--> band — ahead of every other use. The reserve goes with it: overruns used to come out of fit-out scope and never out of the working-capital reserve, but there is no residual reserve left to protect — the bail line is <!--m:bailLineK-->$0K<!--/m-->/yr. None of this is at stake before the pilot verdict (D16-r), which is what makes it a design finding now instead of a loss later.
